...
def initialize_runtime(
    ...
    action = CmdRunAction(command='git checkout -b wsai_code')
    action.set_hard_timeout(600)
    logger.info(action, extra={'msg_type': 'ACTION'})
    obs = runtime.run_action(action)
    logger.info(obs, extra={'msg_type': 'OBSERVATION'})
    assert_and_raise(
        obs.exit_code == 0, f'Failed to git checkout new branch wsai_code: {str(obs)}'
    )
...
def complete_runtime(
    ...
    action = CmdRunAction(command='git commit -m "wsai_code edits"')
    action.set_hard_timeout(600)
    logger.info(action, extra={'msg_type': 'ACTION'})
    obs = runtime.run_action(action)
    logger.info(obs, extra={'msg_type': 'OBSERVATION'})
    assert_and_raise(
        ...
        f'Failed to git commit -m "wsai_code": {str(obs)}',
    )
