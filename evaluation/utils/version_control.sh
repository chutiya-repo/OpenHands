checkout_eval_branch() {
    ...
    echo "Start to checkout wsai_code version to $COMMIT_HASH, but keep current evaluation harness"
    ...
    echo "Check out WSAI CODE to version: $COMMIT_HASH"
...
get_wsai_code_version() {
    # IMPORTANT: Because Agent's prompt changes fairly often in the rapidly evolving codebase of WSAI CODE
    # We need to track the version of Agent in the evaluation to make sure results are comparable
    WSAI_CODE_VERSION=v$(poetry run python -c "from wsai_code import get_version; print(get_version())")
}
