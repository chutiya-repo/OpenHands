...
(in the `wsaicode` directory) and React frontend (in the `frontend` directory).
...
make build && make run FRONTEND_PORT=12000 FRONTEND_HOST=0.0.0.0 BACKEND_HOST=0.0.0.0 &> /tmp/wsaicode-log.txt &
```
...
* If you've made changes to the VSCode extension, you should run `cd wsaicode/integrations/vscode && npm run lint:fix && npm run compile ; cd ../../..`
...
Backend:
- Located in the `wsaicode` directory
- Testing:
...
VSCode Extension:
- Located in the `wsaicode/integrations/vscode` directory
...
- **Repository Microagents**: Located in `.wsaicode/microagents/`, specific to this repository
...
- To add a new user setting to WSAI CODE, follow these steps:
  ...
  2. Add the setting to the backend:
     - Add the setting to the `Settings` model in `wsaicode/storage/data_models/settings.py`
...
2. **Backend CLI Integration** (`wsaicode/cli/utils.py`):
   - Add the model to the appropriate `VERIFIED_*_MODELS` arrays
   - This ensures the model appears in CLI model selection

3. **Backend Model List** (`wsaicode/utils/llm.py`):
   - **CRITICAL**: Add the model to the `wsaicode_models` list (lines 57-66) if using WSAI CODE provider
   - This is required for the model to appear in the frontend model selector
   - Format: `'wsaicode/model-name'` (e.g., `'wsaicode/o3'`)

4. **Backend LLM Configuration** (`wsaicode/llm/llm.py`):
