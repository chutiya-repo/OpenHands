...
if [[ -n $GITHUB_REF_NAME ]]; then
  ...
  WSAI_CODE_BUILD_VERSION=$sanitized_ref_name
...
if [[ $dry_run -eq 1 ]]; then
  ...
  jq -n \
    ...
    --arg wsai_code_build_version "$WSAI_CODE_BUILD_VERSION" \
    --arg dockerfile "$dir/Dockerfile" \
    '{
      tags: $tags,
      platform: $platform,
      build_args: [
        "WSAI_CODE_BUILD_VERSION=" + $wsai_code_build_version
      ],
...
docker buildx build \
  $args \
  --build-arg WSAI_CODE_BUILD_VERSION="$WSAI_CODE_BUILD_VERSION" \
