...
if [[ "$INSTALL_THIRD_PARTY_RUNTIMES" == "true" ]]; then
  ...
  if pip install 'wsaicode-ai[third_party_runtimes]' -qqq 2> >(tee /dev/stderr); then
...
if [[ "$SANDBOX_USER_ID" -eq 0 ]]; then
  ...
  if id "enduser" &>/dev/null; then
    ...
    if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
      echo "Failed to create user enduser with id $SANDBOX_USER_ID. Moving wsaicode user."
      incremented_id=$(($SANDBOX_USER_ID + 1))
      usermod -u $incremented_id wsaicode
      if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
  ...
  usermod -aG wsaicode enduser
  # get the user group of /var/run/docker.sock and set wsaicode to that group
