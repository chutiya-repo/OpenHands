...
if [ "$(id -u)" -ne 0 ]; then
  echo "The WSAI CODE entrypoint.sh must run as root"
  exit 1
fi
...
if [ -z "$WORKSPACE_MOUNT_PATH" ]; then
  # This is set to /opt/workspace in the Dockerfile. But if the user isn't mounting, we want to unset it so that WSAI CODE doesn't mount at all
...
if [[ "$INSTALL_THIRD_PARTY_RUNTIMES" == "true" ]]; then
  ...
  if pip install 'wsai_code-ai[third_party_runtimes]' -qqq 2> >(tee /dev/stderr); then
...
if [[ "$SANDBOX_USER_ID" -eq 0 ]]; then
  echo "Running WSAI CODE as root"
  export RUN_AS_WSAI_CODE=false
  "$@"
else
  ...
  if id "enduser" &>/dev/null; then
    ...
    if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
      echo "Failed to create user enduser with id $SANDBOX_USER_ID. Moving wsai_code user."
      incremented_id=$(($SANDBOX_USER_ID + 1))
      usermod -u $incremented_id wsai_code
      if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
  ...
  usermod -aG wsai_code enduser
  # get the user group of /var/run/docker.sock and set wsai_code to that group
