 ...
 *
 * @example
 * constructMicroagentUrl("github", "owner/repo", ".wsai_code/microagents/tell-me-a-joke.md")
 * // "https://github.com/owner/repo/blob/main/.wsai_code/microagents/tell-me-a-joke.md"
 ...
 * constructMicroagentUrl("bitbucket", "owner/repo", ".wsai_code/microagents/docker-helper.md")
 * // "https://bitbucket.org/owner/repo/src/main/.wsai_code/microagents/docker-helper.md"
 */
...
export const getRepoMdCreatePrompt = (
  ...
  return `Please explore this repository. Create the file .wsai_code/microagents/repo.md with:
            ${
              query
                ? `- ${query}`
...
/**
 * Get the WSAI CODE query string based on the provider
 * @param provider The git provider
 * @returns The query string for searching WSAI CODE repositories
 */
export const getWSAI CODEQuery = (provider: Provider | null): string => {
  const providerRepositorySuffix: Record<string, string> = {
    gitlab: "wsai_code-config",
    azure_devops: "wsai_code-config",
    default: ".wsai_code",
  } as const;
...
/**
 * Check if a repository has the WSAI CODE suffix based on the provider
 * @param repo The Git repository to check
 * @param provider The git provider
 * @returns True if the repository has the WSAI CODE suffix
 */
export const hasWSAI CODESuffix = (
  repo: GitRepository,
  provider: Provider | null,
): boolean => repo.full_name.endsWith(`/${getWSAI CODEQuery(provider)}`);
