...
export function MicroagentManagementSidebar({
  ...
  const {
    ...
    getWSAI CODEQuery(selectedProvider),
    selectedProvider,
    false,
    500,
  );
  ...
  useEffect(() => {
    ...
    if (userAndOrgLevelRepositorySearchResults?.length) {
      userAndOrgLevelRepositorySearchResults.forEach((repo: GitRepository) => {
        if (
          hasWSAI CODESuffix(repo, selectedProvider) &&
  ...
  // Process other repositories (non-WSAI CODE repositories) from filteredRepositories
  useEffect(() => {
    ...
    filteredRepositories.forEach((repo: GitRepository) => {
      // Only include repositories that don't have the WSAI CODE suffix
      if (!hasWSAI CODESuffix(repo, selectedProvider)) {
