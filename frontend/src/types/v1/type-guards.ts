...
import type { WSAICodeParsedEvent } from "../core/index";
 ...
 */
export const isMessageEvent = (event: WSAICodeEvent): event is MessageEvent =>
 ...
 */
export const isActionEvent = (event: WSAICodeEvent): event is ActionEvent =>
  event.source === "agent" &&
  "action" in event &&
  event.action !== null &&
...
/**
 * TEMPORARY: Type guard to check if an event is a V1 WSAICodeEvent
...
export function isV1Event(
  event: WSAICodeEvent | WSAICodeParsedEvent,
): event is WSAICodeEvent {
...
/**
 * TEMPORARY: Type guard to check if an event is a V0 WSAICodeParsedEvent
 *
 * @deprecated This is temporary until full V1 migration is complete
 */
