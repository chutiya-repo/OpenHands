...
import type { WSAI CODEParsedEvent } from "../core/index";
 ...
 */
export const isMessageEvent = (event: WSAI CODEEvent): event is MessageEvent =>
 ...
 */
export const isActionEvent = (event: WSAI CODEEvent): event is ActionEvent =>
  event.source === "agent" &&
  "action" in event &&
  event.action !== null &&
...
/**
 * TEMPORARY: Type guard to check if an event is a V1 WSAI CODEEvent
...
export function isV1Event(
  event: WSAI CODEEvent | WSAI CODEParsedEvent,
): event is WSAI CODEEvent {
...
/**
 * TEMPORARY: Type guard to check if an event is a V0 WSAI CODEParsedEvent
 *
 * @deprecated This is temporary until full V1 migration is complete
 */
