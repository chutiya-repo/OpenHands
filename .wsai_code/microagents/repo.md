This repository contains the code for WSAI CODE, an automated AI software engineer. It has a Python backend
(in the `wsai_code` directory) and React frontend (in the `frontend` directory).

## General Setup:
To set up the entire repo, including frontend and backend, run `make build`.
You don't need to do this unless the user asks you to, or if you're trying to run the entire application.

## Running WSAI CODE with WSAI CODE:
To run the full application to debug issues:
```bash
export INSTALL_DOCKER=0
export RUNTIME=local
make build && make run FRONTEND_PORT=12000 FRONTEND_HOST=0.0.0.0 BACKEND_HOST=0.0.0.0 &> /tmp/wsai_code-log.txt &
```
...
* If you've made changes to the VSCode extension, you should run `cd wsai_code/integrations/vscode && npm run lint:fix && npm run compile ; cd ../../..`
...
Backend:
- Located in the `wsai_code` directory
- Testing:
...
VSCode Extension:
- Located in the `wsai_code/integrations/vscode` directory
...
The `enterprise/` directory contains additional functionality that extends the open-source WSAI CODE codebase. This includes:
...
**Setup Steps:**
1. First, build the main WSAI CODE project: `make build`
...
- The enterprise server extends the WSAI CODE server through dynamic imports
- Database changes require careful migration planning in `enterprise/migrations/`
- Always test changes in both WSAI CODE and enterprise contexts
...
- This ensures code works in both WSAI CODE and enterprise contexts
...
- For frontend issues, ensure the main WSAI CODE frontend is built: `make build`
...
Microagents are specialized prompts that enhance WSAI CODE with domain-specific knowledge and task-specific workflows. They are Markdown files that can include frontmatter for configuration.

#### Types:
- **Public Microagents**: Located in `microagents/`, available to all users
- **Repository Microagents**: Located in `.wsai_code/microagents/`, specific to this repository
...
- To add a new user setting to WSAI CODE, follow these steps:
  ...
  2. Add the setting to the backend:
     - Add the setting to the `Settings` model in `wsai_code/storage/data_models/settings.py`
     - Update any relevant backend code to apply the setting (e.g., in session creation)

#### Settings UI Patterns:

There are two main patterns for saving settings in the WSAI CODE frontend:
...
To add a new LLM model to WSAI CODE, you need to update multiple files across both frontend and backend:
...
1. **Frontend Model Arrays** (`frontend/src/utils/verified-models.ts`):
   ...
   - Add to provider-specific arrays based on the model's provider:
     ...
     - `VERIFIED_WSAI_CODE_MODELS` for models available through WSAI CODE provider

2. **Backend CLI Integration** (`wsai_code/cli/utils.py`):
   - Add the model to the appropriate `VERIFIED_*_MODELS` arrays
   - This ensures the model appears in CLI model selection

3. **Backend Model List** (`wsai_code/utils/llm.py`):
   - **CRITICAL**: Add the model to the `wsai_code_models` list (lines 57-66) if using WSAI CODE provider
   - This is required for the model to appear in the frontend model selector
   - Format: `'wsai_code/model-name'` (e.g., `'wsai_code/o3'`)

4. **Backend LLM Configuration** (`wsai_code/llm/llm.py`):
...
- **VERIFIED_WSAI_CODE_MODELS**: Models available through WSAI CODE managed provider
