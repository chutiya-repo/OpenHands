# WSAI CODE Enterprise Usage Telemetry Service
...
5. [Implementation Plan](#5-implementation-plan)
   - 5.1 [Database Schema and Models (M1)](#51-database-schema-and-models-m1)
     - 5.1.1 [WSAI CODE - Database Migration](#511-wsai_code---database-migration)
     - 5.1.2 [WSAI CODE - Model Tests](#512-wsai_code---model-tests)
   - 5.2 [Metrics Collection Framework (M2)](#52-metrics-collection-framework-m2)
     - 5.2.1 [WSAI CODE - Core Collection Framework](#521-wsai_code---core-collection-framework)
     - 5.2.2 [WSAI CODE - Example Collectors](#522-wsai_code---example-collectors)
     - 5.2.3 [WSAI CODE - Framework Tests](#523-wsai_code---framework-tests)
   - 5.3 [Collection and Upload Processors (M3)](#53-collection-and-upload-processors-m3)
     - 5.3.1 [WSAI CODE - Collection Processor](#531-wsai_code---collection-processor)
     - 5.3.2 [WSAI CODE - Upload Processor](#532-wsai_code---upload-processor)
     - 5.3.3 [WSAI CODE - Integration Tests](#533-wsai_code---integration-tests)
   - 5.4 [License Warning API (M4)](#54-license-warning-api-m4)
     - 5.4.1 [WSAI CODE - License Status API](#541-wsai_code---license-status-api)
     - 5.4.2 [WSAI CODE - API Integration](#542-wsai_code---api-integration)
   - 5.5 [UI Warning Banner (M5)](#55-ui-warning-banner-m5)
     - 5.5.1 [WSAI CODE - UI Warning Banner](#551-wsai_code---ui-warning-banner)
     - 5.5.2 [WSAI CODE - UI Integration](#552-wsai_code---ui-integration)
   - 5.6 [Helm Chart Deployment Configuration (M6)](#56-helm-chart-deployment-configuration-m6)
     - 5.6.1 [WSAI CODE-Cloud - Cronjob Manifests](#561-wsai_code-cloud---cronjob-manifests)
     - 5.6.2 [WSAI CODE-Cloud - Configuration Management](#562-wsai_code-cloud---configuration-management)
   - 5.7 [Documentation and Enhanced Collectors (M7)](#57-documentation-and-enhanced-collectors-m7)
     - 5.7.1 [WSAI CODE - Advanced Collectors](#571-wsai_code---advanced-collectors)
     - 5.7.2 [WSAI CODE - Monitoring and Testing](#572-wsai_code---monitoring-and-testing)
     - 5.7.3 [WSAI CODE - Technical Documentation](#573-wsai_code---technical-documentation)

## 1. Introduction

### 1.1 Problem Statement

WSAI CODE Enterprise (OHE) helm charts are publicly available but not open source, creating a visibility gap for the sales team. Unknown users can install and use OHE without the vendor's knowledge, preventing proper customer engagement and sales pipeline management. Without usage telemetry, the vendor cannot identify potential customers, track installation health, or proactively support users who may need assistance.
...
When telemetry uploads fail for more than 4 days, users will see a prominent warning banner in the WSAI CODE Enterprise UI:

```
⚠️ Your WSAI CODE Enterprise license will expire in 30 days. Please contact support if this issue persists.
```
...
2. **Generates unique customer identifiers** using administrator contact information:
   - Customer email: Determined by the following priority order:
     1. `WSAI_CODE_ADMIN_EMAIL` environment variable (if set in helm values)
...
**Tier 1: Explicit Configuration** - The `WSAI_CODE_ADMIN_EMAIL` environment variable allows administrators to explicitly specify the contact email during deployment.
...
class TelemetryUploadProcessor(MaintenanceTaskProcessor):
    ...
    def _get_admin_email(self) -> str | None:
        ...
        env_admin_email = os.getenv('WSAI_CODE_ADMIN_EMAIL')
        if env_admin_email:
...
async def get_license_status():
    ...
    if not _is_wsai_code_enterprise():
    ...
    with session_maker() as session:
        ...
        if not last_upload:
            ...
            return {
                "warn": True,
                "message": "WSAI CODE Enterprise license verification pending. Please ensure network connectivity."
            }
        ...
        if days_since_upload > 4:
            ...
            if oldest_unsent:
                ...
                if days_until_expiration <= 0:
                    message = "Your WSAI CODE Enterprise license has expired. Please contact support immediately."
                else:
                    message = f"Your WSAI CODE Enterprise license will expire in {days_until_expiration} days. Please contact support if this issue persists."

                return {"warn": True, "message": message}

        return {"warn": False, "message": ""}

def _is_wsai_code_enterprise() -> bool:
...
The cronjob configurations will be deployed via the WSAI CODE-Cloud helm charts.
...
```yaml
# charts/wsai_code/templates/telemetry-collection-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "wsai_code.fullname" . }}-telemetry-collection
  labels:
    {{- include "wsai_code.labels" . | nindent 4 }}
spec:
      ...
      template:
        spec:
          ...
          - name: telemetry-collector
            ...
            env:
            {{- include "wsai_code.env" . | nindent 12 }}
            command:
            - python
            - -c
            - |
...
```yaml
# charts/wsai_code/templates/telemetry-upload-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "wsai_code.fullname" . }}-telemetry-upload
  labels:
    {{- include "wsai_code.labels" . | nindent 4 }}
spec:
        ...
        spec:
          ...
          - name: telemetry-uploader
            ...
            - name: REPLICATED_PUBLISHABLE_KEY
              valueFrom:
                secretKeyRef:
                  name: {{ include "wsai_code.fullname" . }}-replicated-config
                  key: publishable-key
            - name: REPLICATED_APP_SLUG
              value: {{ .Values.telemetry.replicatedAppSlug | default "wsai_code-enterprise" | quote }}
            command:
            - python
            - -c
            - |
              ...
              processor = TelemetryUploadProcessor(
                  ...
                  replicated_app_slug=os.getenv('REPLICATED_APP_SLUG', 'wsai_code-enterprise')
              )
              task = MaintenanceTask()
              task.set_processor(processor)
              task.status = MaintenanceTaskStatus.PENDING
...
#### 5.2.1 WSAI CODE - Core Collection Framework
...
#### 5.2.2 WSAI CODE - Example Collectors
...
#### 5.2.3 WSAI CODE - Framework Tests
...
#### 5.5.1 WSAI CODE - UI Warning Banner
...
#### 5.5.2 WSAI CODE - UI Integration
...
**Repository**: WSAI CODE-Cloud
Create Kubernetes cronjob configurations and deployment scripts.

#### 5.6.1 WSAI CODE-Cloud - Cronjob Manifests

- [ ] `charts/wsai_code/templates/telemetry-collection-cronjob.yaml`
- [ ] `charts/wsai_code/templates/telemetry-upload-cronjob.yaml`

#### 5.6.2 WSAI CODE-Cloud - Configuration Management

- [ ] `charts/wsai_code/templates/replicated-secret.yaml`
- [ ] Update `charts/wsai_code/values.yaml` with telemetry configuration options:
  ```yaml
  # Add to values.yaml
  telemetry:
    enabled: true
    replicatedAppSlug: "wsai_code-enterprise"
    adminEmail: ""  # Optional: admin email for customer identification

  # Add to deployment environment variables
  env:
    WSAI_CODE_ADMIN_EMAIL: "{{ .Values.telemetry.adminEmail }}"
  ```
...
#### 5.7.1 WSAI CODE - Advanced Collectors
...
#### 5.7.2 WSAI CODE - Monitoring and Testing
...
#### 5.7.3 WSAI CODE - Technical Documentation
