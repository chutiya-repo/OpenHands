...
from wsai_code.events import Event, EventSource
from wsai_code.events.action import (
    AgentFinishAction,
    MessageAction,
)
from wsai_code.events.event_filter import EventFilter
from wsai_code.events.event_store_abc import EventStoreABC
...
WSAI_CODE_RESOLVER_TEMPLATES_DIR = (
    ...
    or 'wsai_code/integrations/templates/resolver/'
)
...
def get_oh_labels(web_host: str) -> tuple[str, str]:
    ...
    Returns:
        ...
        - oh_label is 'wsai_code-exp' for staging/local hosts, 'wsai_code' otherwise
        - inline_oh_label is '@wsai_code-exp' for staging/local hosts, '@wsai_code' otherwise
    """
    web_host = web_host.strip()
    is_staging_or_local = 'staging' in web_host or 'local' in web_host
    oh_label = 'wsai_code-exp' if is_staging_or_local else 'wsai_code'
    inline_oh_label = '@wsai_code-exp' if is_staging_or_local else '@wsai_code'
...
def has_exact_mention(text: str, mention: str) -> bool:
    ...
    Args:
        ...
        mention: The mention to look for (e.g. "@wsai_code")

    Returns:
        bool: True if the exact mention is found, False otherwise

    Example:
        >>> has_exact_mention("Hello @wsai_code!", "@wsai_code")  # True
        >>> has_exact_mention("Hello @wsai_code-agent!", "@wsai_code")  # False
        >>> has_exact_mention("(@wsai_code)", "@wsai_code")  # True
        >>> has_exact_mention("user@wsai_code.com", "@wsai_code")  # False
        >>> has_exact_mention("Hello @WSAI CODE!", "@wsai_code")  # True (case-insensitive)
    """
