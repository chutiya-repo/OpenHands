...
from wsai_code.app_server.config import get_app_conversation_service
...
from wsai_code.integrations.provider import ProviderHandler, ProviderType
from wsai_code.sdk import TextContent
...
class SlackUnkownUserView(SlackViewInterface):
    ...
    user_msg: str | None
    slack_user_id: str
    slack_to_wsai_code_user: SlackUser | None
...
class SlackNewConversationView(SlackViewInterface):
    ...
    user_msg: str | None
    slack_user_id: str
    slack_to_wsai_code_user: SlackUser
    ...
    def _get_instructions(self, jinja_env: Environment) -> tuple[str, str]:
        ...
        user_info: SlackUser = self.slack_to_wsai_code_user
    ...
    async def save_slack_convo(self, v1_enabled: bool = False):
        if self.slack_to_wsai_code_user:
            user_info: SlackUser = self.slack_to_wsai_code_user
    ...
    async def create_or_update_conversation(self, jinja: Environment) -> str:
        ...
        self.v1_enabled = await is_v1_enabled_for_slack_resolver(
            self.slack_to_wsai_code_user.keycloak_user_id
        )
    ...
    async def _create_v0_conversation(
        ...
        agent_loop_info = await create_new_conversation(
            user_id=self.slack_to_wsai_code_user.keycloak_user_id,
    ...
    async def _create_v1_conversation(self, jinja: Environment) -> None:
        ...
        start_request = AppConversationStartRequest(
            ...
            selected_repository=self.selected_repo,
            git_provider=git_provider,
            title=f'Slack conversation from {self.slack_to_wsai_code_user.slack_display_name}',
...
class SlackUpdateExistingConversationView(SlackNewConversationView):
    ...
    async def send_message_to_v1_conversation(self, jinja: Environment):
        ...
        from wsai_code.agent_server.models import SendMessageRequest
        from wsai_code.app_server.config import (
            get_app_conversation_info_service,
            get_httpx_client,
            get_sandbox_service,
        )
        from wsai_code.app_server.event_callback.util import (
            ensure_conversation_found,
            get_agent_server_url_from_sandbox,
        )
        from wsai_code.app_server.services.injector import InjectorState
        from wsai_code.app_server.user.specifiy_user_context import (
            ADMIN,
            USER_CONTEXT_ATTR,
        )
