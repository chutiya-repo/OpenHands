...
from wsaicode.server.session import Session
...
from wsaicode.utils.shutdown_listener import should_continue
...
RUNTIME_URL_PATTERN = os.getenv(
    'RUNTIME_URL_PATTERN', 'https://{runtime_id}.prod-runtime.wsai-code.dev'
)
...
class SaasNestedConversationManager(ConversationManager):
    ...
    async def _setup_experiment_config(
        ...
        from wsaicode.experiments.experiment_manager import (
            ExperimentConfig,
    ...
    def _get_mcp_config(self, user_id: str) -> MCPConfig | None:
        ...
        if not mcp_api_key:
            return None
        web_host = os.environ.get('WEB_HOST', 'app.wsai-code.dev')
        shttp_servers = [
    ...
    async def _create_runtime(
        ...
        env_vars['USER'] = (
            RUNTIME_USERNAME
            if RUNTIME_USERNAME
            else ('wsaicode' if config.run_as_wsaicode else 'root')
        )
        ...
        env_vars['FILE_STORE_PATH'] = '/workspace/.wsaicode/file_store'
