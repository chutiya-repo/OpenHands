...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('wsai_code_helped_author', sa.Boolean(), nullable=True),
        sa.Column('num_wsai_code_commits', sa.Integer(), nullable=True),
        sa.Column('num_wsai_code_review_comments', sa.Integer(), nullable=True),
        sa.Column('num_wsai_code_general_comments', sa.Integer(), nullable=True),
    )

    # Create indexes for efficient querying
    op.create_index(
        'ix_wsai_code_prs_repo_id', 'wsai_code_prs', ['repo_id'], unique=False
    )
    op.create_index(
        'ix_wsai_code_prs_pr_number', 'wsai_code_prs', ['pr_number'], unique=False
    )
    op.create_index(
        'ix_wsai_code_prs_status', 'wsai_code_prs', ['status'], unique=False
    )

    # Create unique constraint on repo_id + pr_number combination
    op.create_index(
        'ix_wsai_code_prs_repo_pr_unique',
        'wsai_code_prs',
        ['repo_id', 'pr_number'],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index('ix_wsai_code_prs_repo_id', table_name='wsai_code_prs')
    op.drop_index('ix_wsai_code_prs_pr_number', table_name='wsai_code_prs')
    op.drop_index('ix_wsai_code_prs_status', table_name='wsai_code_prs')
    op.drop_index('ix_wsai_code_prs_repo_pr_unique', table_name='wsai_code_prs')
    op.drop_table('wsai_code_prs')
