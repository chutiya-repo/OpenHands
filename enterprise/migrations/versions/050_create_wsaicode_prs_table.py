...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('wsaicode_helped_author', sa.Boolean(), nullable=True),
        sa.Column('num_wsaicode_commits', sa.Integer(), nullable=True),
        sa.Column('num_wsaicode_review_comments', sa.Integer(), nullable=True),
        sa.Column('num_wsaicode_general_comments', sa.Integer(), nullable=True),
    )

    # Create indexes for efficient querying
    op.create_index(
        'ix_wsaicode_prs_repo_id', 'wsaicode_prs', ['repo_id'], unique=False
    )
    op.create_index(
        'ix_wsaicode_prs_pr_number', 'wsaicode_prs', ['pr_number'], unique=False
    )
    op.create_index(
        'ix_wsaicode_prs_status', 'wsaicode_prs', ['status'], unique=False
    )

    # Create unique constraint on repo_id + pr_number combination
    op.create_index(
        'ix_wsaicode_prs_repo_pr_unique',
        'wsaicode_prs',
        ['repo_id', 'pr_number'],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index('ix_wsaicode_prs_repo_id', table_name='wsaicode_prs')
    op.drop_index('ix_wsaicode_prs_pr_number', table_name='wsaicode_prs')
    op.drop_index('ix_wsaicode_prs_status', table_name='wsaicode_prs')
    op.drop_index('ix_wsaicode_prs_repo_pr_unique', table_name='wsaicode_prs')
    op.drop_table('wsaicode_prs')
