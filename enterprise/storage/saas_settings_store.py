...
from wsaicode.server.settings import Settings
...
class SaasSettingsStore(SettingsStore):
    ...
    async def store(self, item: Settings):
        ...
        if item and self._is_wsaicode_provider(item):
            await self._ensure_wsaicode_api_key(item)
    ...
    def _is_wsaicode_provider(self, item: Settings) -> bool:
        """Check if the settings use the WSAI CODE provider."""
        return bool(item.llm_model and item.llm_model.startswith('wsaicode/'))

    async def _ensure_wsaicode_api_key(self, item: Settings) -> None:
        ...
        generated_key = await self._generate_wsaicode_key()
        if generated_key:
            item.llm_api_key = SecretStr(generated_key)
            logger.info(
                'saas_settings_store:store:generated_wsaicode_key',
                extra={'user_id': self.user_id},
            )
        else:
            logger.warning(
                'saas_settings_store:store:failed_to_generate_wsaicode_key',
                extra={'user_id': self.user_id},
            )
    ...
    async def _generate_wsaicode_key(self) -> str | None:
        """Generate a new WSAI CODE provider key for a user."""
        if not (LITE_LLM_API_KEY and LITE_LLM_API_URL):
            logger.warning(
                'saas_settings_store:_generate_wsaicode_key:litellm_config_not_found',
                extra={'user_id': self.user_id},
            )
            return None
        ...
        try:
            async with httpx.AsyncClient(
                ...
                if key:
                    logger.info(
                        'saas_settings_store:_generate_wsaicode_key:success',
                        extra={
                            'user_id': self.user_id,
                    ...
                    logger.error(
                        'saas_settings_store:_generate_wsaicode_key:no_key_in_response',
                        extra={'user_id': self.user_id, 'response_json': response_json},
                    )
                    return None
        except Exception as e:
            logger.exception(
                'saas_settings_store:_generate_wsaicode_key:error',
