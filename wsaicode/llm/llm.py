...
from wsaicode.llm.model_features import get_features
...
from wsaicode.core.exceptions import LLMNoResponseError
from wsaicode.core.logger import wsaicode_logger as logger
from wsaicode.core.message import Message
from wsaicode.llm.debug_mixin import DebugMixin
from wsaicode.llm.fn_call_converter import (
    STOP_WORDS,
    convert_fncall_messages_to_non_fncall_messages,
    convert_non_fncall_messages_to_fncall_messages,
)
from wsaicode.llm.retry_mixin import RetryMixin
...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        # Handle WSAI CODE provider - rewrite to litellm_proxy
        if self.config.model.startswith('wsaicode/'):
            model_name = self.config.model.removeprefix('wsaicode/')
            self.config.model = f'litellm_proxy/{model_name}'
            self.config.base_url = 'https://llm-proxy.app.wsai-code.dev/'
            logger.debug(
                f'Rewrote wsaicode/{model_name} to {self.config.model} with base URL {self.config.base_url}'
            )
        ...
        # Azure issue: https://github.com/wsaicode/wsaicode/issues/6777
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if mock_function_calling and 'tools' in kwargs:
                ...
                if (
                    'wsaicode-lm' in self.config.model
                ...
                if 'wsaicode-lm' in self.config.model:
                    # If we don't have this, we might run into issue when serving wsaicode-lm
                    # using SGLang
    ...
    def _supports_vision(self) -> bool:
        ...
        if os.getenv('WSAI_CODE_FORCE_VISION', '').lower() in (
            '1',
            'true',
            'yes',
            'on',
