...
from wsaicode.integrations.azure_devops.azure_devops_service import (
    AzureDevOpsServiceImpl,
)
from wsaicode.integrations.bitbucket.bitbucket_service import BitBucketServiceImpl
...
from wsaicode.integrations.provider import ProviderToken
from wsaicode.integrations.service_types import GitService, ProviderType
from wsaicode.server.shared import ConversationStoreImpl, config, server_config
...
HOST = f'https://{os.getenv("WEB_HOST", "app.wsai-code.dev").strip()}'
...
async def get_conversation_link(
    ...
    """Appends a followup link, in the PR body, to the WSAI CODE conversation that opened the PR"""
...
async def create_pr(
    ...
    """Open a PR in GitHub"""
    logger.info('Calling WSAI CODE MCP create_pr')

    request = get_http_request()
    headers = request.headers
    conversation_id = headers.get('X-WSAI CODE-ServerConversation-ID', None)
...
async def create_mr(
    ...
    """Open a MR in GitLab"""
    logger.info('Calling WSAI CODE MCP create_mr')
...
async def create_bitbucket_pr(
    ...
    logger.info('Calling WSAI CODE MCP create_bitbucket_pr')
...
async def create_azure_devops_pr(
    ...
    logger.info('Calling WSAI CODE MCP create_azure_devops_pr')
