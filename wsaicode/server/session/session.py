...
from wsaicode.core.config.mcp_config import WSAICodeMCPConfigImpl
from wsaicode.core.exceptions import MicroagentValidationError
from wsaicode.core.logger import WSAICodeLoggerAdapter
from wsaicode.core.schema import AgentState
from wsaicode.events.action import MessageAction, NullAction
...
from wsaicode.server.session.agent_session import AgentSession
...
class WebSession:
    ...
    (see wsaicode.server.session.__init__) so downstream imports/tests continue to
    ...
    def __init__(
        ...
        self.logger = WSAICodeLoggerAdapter(extra={'session_id': sid})
    ...
    async def initialize_agent(
        ...
        wsaicode_mcp_server, wsaicode_mcp_stdio_servers = (
            WSAICodeMCPConfigImpl.create_default_mcp_server_config(
                self.config.mcp_host, self.config, self.user_id
            )
        )

        if wsaicode_mcp_server:
            self.config.mcp.shttp_servers.append(wsaicode_mcp_server)
            self.logger.debug('Added default MCP HTTP server to config')

            self.config.mcp.stdio_servers.extend(wsaicode_mcp_stdio_servers)
...
# wsaicode.server.session.session import Session
Session = WebSession
