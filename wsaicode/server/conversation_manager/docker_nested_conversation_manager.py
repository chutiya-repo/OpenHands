...
from wsaicode.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderHandler
...
from wsaicode.server.config.server_config import ServerConfig
from wsaicode.server.constants import ROOM_KEY
...
from wsaicode.server.session.session import WebSession as Session
from wsaicode.storage.conversation.conversation_store import ConversationStore
from wsaicode.storage.data_models.conversation_metadata import ConversationMetadata
...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def get_running_agent_loops(
        ...
        conversation_ids = {
            name[len('wsaicode-runtime-') :]
            for name in names
            if name.startswith('wsaicode-runtime-')
        }
    ...
    async def close_session(self, sid: str):
        ...
        try:
            container = self.docker_client.containers.get(f'wsaicode-runtime-{sid}')
    ...
    async def get_agent_loop_info(
        ...
        for container in containers:
            if not container.name or not container.name.startswith(
                ...
                continue
            conversation_id = container.name[len('wsaicode-runtime-') :]
    ...
    def get_nested_url_for_container(self, container: Container) -> str:
        ...
        conversation_id = container_name[len('wsaicode-runtime-') :]
    ...
    async def _create_runtime(
        ...
        config: WSAICodeConfig = ExperimentManagerImpl.run_config_variant_test(
        ...
        env_vars['CONVERSATION_MANAGER_CLASS'] = (
            'wsaicode.server.conversation_manager.standalone_conversation_manager.StandaloneConversationManager'
        )
        env_vars['SERVE_FRONTEND'] = '0'
        env_vars['RUNTIME'] = 'local'
        # TODO: In the long term we may come up with a more secure strategy for user management within the nested runtime.
        env_vars['USER'] = 'wsaicode' if config.run_as_wsaicode else 'root'
        ...
        if config.file_store == 'local':
            ...
            volumes.append(
                f'{file_store_path}/{conversation_dir}:/root/.wsaicode/{conversation_dir}:rw'
            )
