...
1. Create a submodule in [security](/wsaicode/security/) with your analyzer's desired name
    * Have your main class inherit from [SecurityAnalyzer](/wsaicode/security/analyzer.py)
    * Optional: define API endpoints for `/api/security/{path:path}` to manage settings,
2. Add your analyzer class to the [options](/wsaicode/security/options.py) to have it be visible from the frontend combobox
