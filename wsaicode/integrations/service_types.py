...
from wsaicode.microagent.types import MicroagentContentResponse, MicroagentResponse
...
class SuggestedTask(BaseModel):
    ...
    def get_prompt_for_task(
        ...
        env = Environment(
            loader=FileSystemLoader('wsaicode/integrations/templates/suggested_task')
        )
...
class BaseGitService(ABC):
    ...
    def _determine_microagents_path(self, repository_name: str) -> str:
        ...
        if actual_repo_name == '.wsaicode' or actual_repo_name == 'wsaicode-config':
            # For repository name ".wsaicode", scan "microagents" folder
            return 'microagents'
        else:
            # Default behavior: look for .wsaicode/microagents directory
            return '.wsaicode/microagents'
