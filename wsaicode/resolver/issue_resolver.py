...
from wsaicode.core.config import AgentConfig, WSAICodeConfig, SandboxConfig
from wsaicode.core.config.utils import load_wsaicode_config
from wsaicode.core.logger import wsaicode_logger as logger
from wsaicode.core.main import create_runtime, run_controller
from wsaicode.events.action import CmdRunAction, MessageAction
...
from wsaicode.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from wsaicode.resolver.issue_handler_factory import IssueHandlerFactory
from wsaicode.resolver.resolver_output import ResolverOutput
from wsaicode.resolver.utils import (
...
from wsaicode.utils.async_utils import GENERAL_TIMEOUT, call_async_from_sync
...
class IssueResolver:
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.app_config = self.update_wsaicode_config(
    ...
    def update_sandbox_config(
        cls,
        wsaicode_config: WSAICodeConfig,
        base_container_image: str | None,
        runtime_container_image: str | None,
        is_experimental: bool,
    ) -> None:
        ...
        if (
            ...
            runtime_container_image = (
                f'ghcr.io/wsaicode/runtime:{wsaicode.__version__}-nikolaik'
            )
        ...
        wsaicode_config.sandbox.base_container_image = (
            sandbox_config.base_container_image
        )
        wsaicode_config.sandbox.runtime_container_image = (
            sandbox_config.runtime_container_image
        )
        wsaicode_config.sandbox.enable_auto_lint = sandbox_config.enable_auto_lint
        wsaicode_config.sandbox.use_host_network = sandbox_config.use_host_network
        wsaicode_config.sandbox.timeout = sandbox_config.timeout
        wsaicode_config.sandbox.local_runtime_url = sandbox_config.local_runtime_url
        wsaicode_config.sandbox.user_id = sandbox_config.user_id
    ...
    def initialize_runtime(
        ...
        logger.info('Checking for .wsaicode/setup.sh script...')
        runtime.maybe_run_setup_script()

        # Setup git hooks if they exist
        logger.info('Checking for .wsaicode/pre-commit.sh script...')
    ...
    async def resolve_issue(
        ...
        if self.repo_instruction is None:
            # Check for .wsaicode_instructions file in the workspace directory
            wsaicode_instructions_path = os.path.join(
                repo_dir, '.wsaicode_instructions'
            )
            if os.path.exists(wsaicode_instructions_path):
                with open(wsaicode_instructions_path, 'r') as f:
