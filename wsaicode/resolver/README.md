...
This tool allows you to use open-source AI agents based on [WSAI CODE](https://github.com/wsaicode/wsaicode)
...
3. Copy `examples/wsaicode-resolver.yml` to your repository's `.github/workflows/` directory
...
6. Set up any [custom configurations required](https://docs.wsaicode.dev/usage/how-to/github-action#custom-configurations)
...
7. Usage:
   ...
   b. Using `@wsaicode-agent` mention:
      - Create a new comment containing `@wsaicode-agent` in any issue
...
Need help? Feel free to [open an issue](https://github.com/wsaicode/wsaicode/issues).
...
```bash
python -m wsaicode.resolver.resolve_issue --selected-repo [OWNER]/[REPO] --issue-number [NUMBER]
```

For instance, if you want to resolve issue #100 in this repo, you would run:

```bash
python -m wsaicode.resolver.resolve_issue --selected-repo wsaicode/wsaicode --issue-number 100
```
...
```bash
poetry run python wsaicode/resolver/resolve_issue.py --selected-repo wsaicode/wsaicode --issue-number 100
```
...
```bash
python -m wsaicode.resolver.send_pull_request --issue-number PR_NUMBER --issue-type pr
```
...
```bash
python -m wsaicode.resolver.visualize_resolver_output --issue-number ISSUE_NUMBER --vis-method json
```
...
```bash
python -m wsaicode.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GIT_USERNAME --pr-type draft
```

If you want to upload to a fork, you can do so by specifying the `fork-owner`:

```bash
python -m wsaicode.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GIT_USERNAME --pr-type draft --fork-owner YOUR_GIT_USERNAME
```

## Providing Custom Instructions

You can customize how the AI agent approaches issue resolution by adding a repository microagent file at `.wsaicode/microagents/repo.md` in your repository. This file's contents will be automatically loaded in the prompt when working with your repository. For more information about repository microagents, see [Repository Instructions](https://github.com/wsaicode/wsaicode/tree/main/microagents#2-repository-instructions-private).

## Troubleshooting

If you have any issues, please open an issue on this GitHub, GitLab, or Bitbucket repo, we're happy to help!
Alternatively, you can [email us](mailto:contact@wsaicode.dev) or join the WSAI CODE Slack workspace (see [the README](/README.md) for an invite link).
