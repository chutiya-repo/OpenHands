...
def should_set_litellm_extra_body(model_name: str) -> bool:
    ...
    Only set litellm_extra_body for wsaicode models to avoid issues
    with providers that don't support extra_body parameters.

    The SDK internally translates "wsaicode/" prefix to "litellm_proxy/"
    ...
    """
    return 'wsaicode/' in model_name or 'litellm_proxy/' in model_name
...
def get_llm_metadata(
    ...
    """
    wsaicode_version = wsaicode.__version__

    metadata: dict[str, Any] = {
        'trace_version': wsaicode_version,
        'tags': [
            'app:wsaicode',
            f'model:{model_name}',
            f'type:{llm_type}',
            f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
            f'wsaicode_version:{wsaicode_version}',
