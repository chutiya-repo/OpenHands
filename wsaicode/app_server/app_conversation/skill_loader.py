...
- User skills from ~/.wsaicode/skills/
...
import wsaicode
from wsaicode.app_server.sandbox.sandbox_models import SandboxInfo
...
from wsaicode.sdk.context.skills import Skill
from wsaicode.sdk.workspace.remote.async_remote_workspace import AsyncRemoteWorkspace

_logger = logging.getLogger(__name__)

# Path to global skills directory
GLOBAL_SKILLS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(wsaicode.__file__)),
    'skills',
)
...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.wsaicode', 'owner')
        - GitLab: ('owner/wsaicode-config', 'owner')
        - Azure DevOps: ('org/wsaicode-config/wsaicode-config', 'org')
    """
    ...
    return org_wsaicode_repo, org_name
...
def _merge_repo_skills_with_precedence(
    ...
    2. .wsaicode/skills/ directory
    3. .wsaicode/microagents/ directory (backward compatibility)

    Args:
        special_skills: Skills from special files in repo root
        skills_dir_skills: Skills from .wsaicode/skills/ directory
        microagents_dir_skills: Skills from .wsaicode/microagents/ directory
...
async def load_repo_skills(
    ...
    2. .md files in .wsaicode/skills/ directory (preferred)
    3. .md files in .wsaicode/microagents/ directory (for backward compatibility)
    ...
    try:
        ...
        # Load .md files from .wsaicode/skills/ directory (preferred)
        skills_dir = f'{repo_root}/.wsaicode/skills'
        skills_dir_skills = await _find_and_load_skill_md_files(
            workspace, skills_dir, working_dir
        )

        # Load .md files from .wsaicode/microagents/ directory (backward compatibility)
        microagents_dir = f'{repo_root}/.wsaicode/microagents'
...
async def _get_org_repository_url(
    org_wsaicode_repo: str, user_context: UserContext
) -> str | None:
    """Get authenticated Git URL for organization repository.

    Args:
        org_wsaicode_repo: Organization repository path
    ...
    """
    try:
        remote_url = await user_context.get_authenticated_git_url(org_wsaicode_repo)
        return remote_url
    except AuthenticationError as e:
        _logger.debug(
            f'org-level skill directory {org_wsaicode_repo} not found: {str(e)}'
        )
        return None
    except Exception as e:
        _logger.debug(
...
async def _clone_org_repository(
    ...
    Args:
        ...
        working_dir: Working directory for command execution
        org_wsaicode_repo: Organization repository path (for logging)
    ...
    if result.exit_code != 0:
        _logger.info(
            f'No org-level skills found at {org_wsaicode_repo} (exit_code: {result.exit_code})'
        )
        _logger.debug(f'Clone command output: {result.stderr}')
        return False

    _logger.info(f'Successfully cloned org-level skills from {org_wsaicode_repo}')
    return True
...
async def load_org_skills(
    ...
    try:
        ...
        org_wsaicode_repo, org_name = await _determine_org_repo_path(
            selected_repository, user_context
        )

        _logger.info(f'Checking for org-level skills at {org_wsaicode_repo}')

        # Get authenticated URL for org repository
        remote_url = await _get_org_repository_url(org_wsaicode_repo, user_context)
        if not remote_url:
            return []

        # Clone the organization repository
        org_repo_dir = f'{working_dir}/_org_wsaicode_{org_name}'
        clone_success = await _clone_org_repository(
            workspace, remote_url, org_repo_dir, working_dir, org_wsaicode_repo
        )
        ...
        _logger.info(
            f'Loaded {len(loaded_skills)} skills from org-level repository {org_wsaicode_repo}: {[s.name for s in loaded_skills]}'
        )
