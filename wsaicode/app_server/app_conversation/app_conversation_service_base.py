...
from wsaicode.sdk.context.agent_context import AgentContext
from wsaicode.sdk.context.condenser import LLMSummarizingCondenser
from wsaicode.sdk.context.skills import load_user_skills
from wsaicode.sdk.llm import LLM
from wsaicode.sdk.security.analyzer import SecurityAnalyzerBase
from wsaicode.sdk.security.confirmation_policy import (
    AlwaysConfirm,
    ConfirmationPolicyBase,
    ConfirmRisky,
    NeverConfirm,
)
from wsaicode.sdk.security.llm_analyzer import LLMSecurityAnalyzer
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        try:
            ...
            # Load user skills from ~/.wsaicode/skills/ directory
            # Uses the SDK's load_user_skills() function which handles loading from
            # ~/.wsaicode/skills/ and ~/.wsaicode/microagents/ (for backward compatibility)
            try:
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            checkout_command = f'git checkout -b {wsaicode_workspace_branch}'
    ...
    async def maybe_run_setup_script(
        ...
        setup_script = workspace.working_dir + '/.wsaicode/setup.sh'
    ...
    async def maybe_setup_git_hooks(
        ...
        command = 'mkdir -p .git/hooks && chmod +x .wsaicode/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if result.get('success'):
                ...
                if 'This hook was installed by WSAI CODE' not in temp_file.read():
