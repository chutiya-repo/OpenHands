"""Configuration for the WSAI CODE App Server."""
...
import wsaicode.app_server.event_callback  # noqa: F401
from wsaicode.agent_server.env_parser import from_env
from wsaicode.app_server.app_conversation.app_conversation_info_service import (
    AppConversationInfoService,
    AppConversationInfoServiceInjector,
)
from wsaicode.app_server.app_conversation.app_conversation_service import (
    AppConversationService,
    AppConversationServiceInjector,
)
from wsaicode.app_server.app_conversation.app_conversation_start_task_service import (
    AppConversationStartTaskService,
    AppConversationStartTaskServiceInjector,
)
from wsaicode.app_server.app_lifespan.app_lifespan_service import AppLifespanService
from wsaicode.app_server.app_lifespan.oss_app_lifespan_service import (
    OssAppLifespanService,
)
from wsaicode.app_server.event.event_service import EventService, EventServiceInjector
from wsaicode.app_server.event_callback.event_callback_service import (
    EventCallbackService,
    EventCallbackServiceInjector,
)
from wsaicode.app_server.sandbox.sandbox_service import (
    SandboxService,
    SandboxServiceInjector,
)
from wsaicode.app_server.sandbox.sandbox_spec_service import (
    SandboxSpecService,
    SandboxSpecServiceInjector,
)
from wsaicode.app_server.services.db_session_injector import (
    DbSessionInjector,
)
from wsaicode.app_server.services.httpx_client_injector import HttpxClientInjector
from wsaicode.app_server.services.injector import InjectorState
from wsaicode.app_server.services.jwt_service import JwtService, JwtServiceInjector
from wsaicode.app_server.user.user_context import UserContext, UserContextInjector
from wsaicode.sdk.utils.models import WSAICodeModel
...
def get_wsaicode_provider_base_url() -> str | None:
    """Return the base URL for the WSAI CODE provider, if configured."""
    return os.getenv('WSAI_CODE_PROVIDER_BASE_URL') or None


def _get_default_lifespan():
    # Check legacy parameters for saas mode. If we are in SAAS mode do not apply
    # WSAI CODE alembic migrations
    if 'saas' in (os.getenv('WSAI_CODE_CONFIG_CLS') or '').lower():
        return None
    return OssAppLifespanService()


class AppServerConfig(WSAICodeModel):
    persistence_dir: Path = Field(default_factory=get_default_persistence_dir)
    web_url: str | None = Field(
        default_factory=get_default_web_url,
        description='The URL where WSAI CODE is running (e.g., http://localhost:3000)',
    )
    wsaicode_provider_base_url: str | None = Field(
        default_factory=get_wsaicode_provider_base_url,
        description='Base URL for the WSAI CODE provider',
    )
...
def config_from_env() -> AppServerConfig:
    # Import defaults...
    from wsaicode.app_server.app_conversation.live_status_app_conversation_service import (  # noqa: E501
        LiveStatusAppConversationServiceInjector,
    )
    from wsaicode.app_server.app_conversation.sql_app_conversation_info_service import (  # noqa: E501
        SQLAppConversationInfoServiceInjector,
    )
    from wsaicode.app_server.app_conversation.sql_app_conversation_start_task_service import (  # noqa: E501
        SQLAppConversationStartTaskServiceInjector,
    )
    from wsaicode.app_server.event.filesystem_event_service import (
        FilesystemEventServiceInjector,
    )
    from wsaicode.app_server.event.google_cloud_event_service import (
        GoogleCloudEventServiceInjector,
    )
    from wsaicode.app_server.event_callback.sql_event_callback_service import (
        SQLEventCallbackServiceInjector,
    )
    from wsaicode.app_server.sandbox.docker_sandbox_service import (
        DockerSandboxServiceInjector,
    )
    from wsaicode.app_server.sandbox.docker_sandbox_spec_service import (
        DockerSandboxSpecServiceInjector,
    )
    from wsaicode.app_server.sandbox.process_sandbox_service import (
        ProcessSandboxServiceInjector,
    )
    from wsaicode.app_server.sandbox.process_sandbox_spec_service import (
        ProcessSandboxSpecServiceInjector,
    )
    from wsaicode.app_server.sandbox.remote_sandbox_service import (
        RemoteSandboxServiceInjector,
    )
    from wsaicode.app_server.sandbox.remote_sandbox_spec_service import (
        RemoteSandboxSpecServiceInjector,
    )
    from wsaicode.app_server.user.auth_user_context import (
