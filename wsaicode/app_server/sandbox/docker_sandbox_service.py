...
from wsaicode.app_server.sandbox.docker_sandbox_spec_service import get_docker_client
...
class DockerSandboxService(SandboxService):
    ...
    async def delete_sandbox(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            try:
                volume_name = f'wsaicode-workspace-{sandbox_id}'
...
class DockerSandboxServiceInjector(SandboxServiceInjector):
    ...
    host_port: int = Field(
        default=3000,
        description=(
            'The port on which the main WSAI CODE app server is running. '
            'Used for webhook callbacks from agent-server containers. '
            'If running WSAI CODE on a non-default port, set this to match. '
