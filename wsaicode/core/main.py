...
from wsaicode.controller.replay import ReplayManager
...
from wsaicode.core.loop import run_agent_until_done
from wsaicode.core.schema import AgentState
from wsaicode.core.setup import (
    create_agent,
    create_controller,
    create_memory,
    create_runtime,
...
from wsaicode.events import EventSource, EventStreamSubscriber
...
from wsaicode.events.observation import AgentStateChangedObservation
from wsaicode.io import read_input, read_task
from wsaicode.mcp import add_mcp_tools_to_agent
...
async def run_controller(
    ...
    It's only used when you launch wsaicode backend directly via cmdline.
    ...
    if agent.config.enable_mcp:
        ...
        _, wsaicode_mcp_stdio_servers = (
            WSAICodeMCPConfigImpl.create_default_mcp_server_config(
                config.mcp_host, config, None
            )
        )
        runtime.config.mcp.stdio_servers.extend(wsaicode_mcp_stdio_servers)
...
if __name__ == '__main__':
    ...
    config: WSAICodeConfig = setup_config_from_args(args)
