...
def log_uncaught_exceptions(
    ...
    wsaicode_logger.error('Uncaught exception', exc_info=(ex_cls, ex, tb))


sys.excepthook = log_uncaught_exceptions
wsaicode_logger = logging.getLogger('wsaicode')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
wsaicode_logger.setLevel(current_log_level)

if DEBUG:
    wsaicode_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    wsaicode_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    wsaicode_logger.addHandler(json_log_handler(current_log_level))
    ...
    wsaicode_logger.addHandler(get_console_handler(current_log_level))

wsaicode_logger.addFilter(SensitiveDataFilter(wsaicode_logger.name))
wsaicode_logger.propagate = False
wsaicode_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of wsaicode/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    wsaicode_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    wsaicode_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        wsaicode_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class WSAICodeLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = wsaicode_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
