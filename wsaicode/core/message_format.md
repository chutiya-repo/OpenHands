...
WSAI CODE uses its own `Message` class (`wsaicode/core/message.py`) which provides rich content support while maintaining compatibility with litellm's message handling system.

## Class Structure

Our `Message` class (`wsaicode/core/message.py`):
