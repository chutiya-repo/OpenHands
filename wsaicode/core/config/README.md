...
WSAI CODE uses a flexible configuration system that allows settings to be defined through environment variables, TOML files, and command-line arguments. The configuration is managed through a package structure in `wsaicode/core/config/`.
...
````python
from wsaicode.core.config import load_app_config
