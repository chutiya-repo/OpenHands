...
from wsaicode.core import logger
from wsaicode.core.config.agent_config import AgentConfig
from wsaicode.core.config.arg_utils import get_headless_parser
...
from wsaicode.core.config.kubernetes_config import KubernetesConfig
...
def load_from_env(
    cfg: WSAICodeConfig, env_or_toml_dict: dict | MutableMapping[str, str]
) -> None:
    ...
    Args:
        cfg: The WSAICodeConfig object to set attributes on.
...
def load_from_toml(cfg: WSAICodeConfig, toml_file: str = 'config.toml') -> None:
    """Load the config from the toml file. Supports both styles of config vars.

    Args:
        cfg: The WSAICodeConfig object to update attributes of.
    ...
    # set LLMSummarizingCondenserConfig as default
        ...
        from wsaicode.core.config.condenser_config import LLMSummarizingCondenserConfig
    ...
    for key in toml_config:
        if key.lower() not in known_sections:
            logger.wsaicode_logger.warning(f'Unknown section [{key}] in {toml_file}')
...
def finalize_config(cfg: WSAICodeConfig) -> None:
...
def get_agent_config_arg(
    ...
    logger.wsaicode_logger.debug(f'Loading agent config from {agent_config_arg}')
    ...
    try:
        ...
        logger.wsaicode_logger.info(f'Config file not found: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.wsaicode_logger.error(
    ...
    logger.wsaicode_logger.debug(f'Loading from toml failed for {agent_config_arg}')
    return None
...
def get_llm_config_arg(
    ...
    if not os.path.exists(toml_file):
        logger.wsaicode_logger.debug(f'Config file not found: {toml_file}')
        return None
...
def get_condenser_config_arg(
    ...
    try:
        ...
        logger.wsaicode_logger.info(f'Config file not found: {toml_file}. Error: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.wsaicode_logger.error(
...
def parse_arguments() -> argparse.Namespace:
    ...
    from wsaicode import get_version
...
def register_custom_agents(config: WSAICodeConfig) -> None:
...
def setup_config_from_args(args: argparse.Namespace) -> WSAICodeConfig:
    ...
    3. ~/.wsaicode/settings.json and ~/.wsaicode/config.toml
    """
    # Load base config from toml and env vars
    config = load_wsaicode_config(config_file=args.config_file)

    # Override with command line arguments if provided
    if args.llm_config:
        logger.wsaicode_logger.debug(f'CLI specified LLM config: {args.llm_config}')
        ...
        if args.llm_config not in config.llms:
            ...
            if llm_config is None and args.config_file != os.path.join(
                os.path.expanduser('~'), '.wsaicode', 'config.toml'
            ):
