...
from wsaicode.core.logger import RollingLogger
from wsaicode.core.logger import wsaicode_logger as logger
from wsaicode.runtime.builder.base import RuntimeBuilder
from wsaicode.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running wsaicode in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official wsaicode app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside wsaicode-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
        ...
        buildx_cmd = [
            ...
            'build',
            '--progress=plain',
            f'--build-arg=WSAI_CODE_RUNTIME_VERSION={get_version()}',
            f'--build-arg=WSAI_CODE_RUNTIME_BUILD_TIME={datetime.datetime.now().isoformat()}',
