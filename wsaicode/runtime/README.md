...
You can learn more about how the runtime works in the [Docker Runtime](https://docs.wsaicode.dev/usage/architecture/runtime) documentation.
...
At the time of this writing, this is mostly used in parallel evaluation, such as this example for [SWE-Bench](https://github.com/wsaicode/wsaicode/tree/main/evaluation/benchmarks/swe_bench#run-inference-on-remoteruntime-experimental).

## Related Components

- The runtime interacts closely with the event system defined in the `wsaicode.events` module.
- It relies on configuration classes from `wsaicode.core.config`.
- Logging is handled through `wsaicode.core.logger`.
