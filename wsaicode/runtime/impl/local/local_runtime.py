...
from wsaicode.core.exceptions import AgentRuntimeDisconnectedError
...
from wsaicode.events.serialization import event_to_dict, observation_from_dict
...
from wsaicode.runtime.impl.docker.docker_runtime import (
    APP_PORT_RANGE_1,
    APP_PORT_RANGE_2,
...
def check_dependencies(code_repo_path: str, check_browser: bool) -> None:
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/wsaicode/wsaicode/blob/main/Development.md to install WSAI CODE.'
...
class LocalRuntime(ActionExecutionClient):
    ...
    Args:
        config (WSAICodeConfig): The application configuration.
    ...
    def __init__(
        ...
        logger.warning(
            ...
            'This is an experimental feature, please report issues to https://github.com/wsaicode/wsaicode/issues. '
            '`run_as_wsaicode` will be ignored since the current user will be used to launch the server. '
        ...
        if self.sid in _RUNNING_SERVERS:
            ...
            if self.config.workspace_base is not None:
                ...
                self._temp_workspace = tempfile.mkdtemp(
                    prefix=f'wsaicode_workspace_{self.sid}',
                )
    ...
    def setup(cls, config: WSAICodeConfig, headless_mode: bool = False):
        ...
        if should_check_dependencies:
            code_repo_path = os.path.dirname(os.path.dirname(wsaicode.__file__))
...
def _create_server(
    ...
    temp_workspace = tempfile.mkdtemp(
        prefix=f'wsaicode_workspace_{workspace_prefix}',
    )
    ...
    env['WSAI_CODE_REPO_PATH'] = code_repo_path
...
def _get_plugins(config: WSAICodeConfig) -> list[PluginRequirement]:
