...
from wsaicode.core.config import WSAICodeConfig, SandboxConfig
from wsaicode.core.config.mcp_config import MCPConfig, MCPStdioServerConfig
from wsaicode.core.exceptions import (
    AgentRuntimeDisconnectedError,
)
from wsaicode.core.logger import wsaicode_logger as logger
from wsaicode.events import EventSource, EventStream, EventStreamSubscriber
...
from wsaicode.events.serialization.action import ACTION_TYPE_TO_CLASS
from wsaicode.integrations.provider import (
    PROVIDER_TOKEN_TYPE,
    ProviderHandler,
    ProviderType,
)
from wsaicode.integrations.service_types import AuthenticationError
from wsaicode.llm.llm_registry import LLMRegistry
from wsaicode.microagent import (
    BaseMicroagent,
    load_microagents_from_dir,
)
from wsaicode.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from wsaicode.runtime.runtime_status import RuntimeStatus
from wsaicode.runtime.utils.edit import FileEditRuntimeMixin
from wsaicode.runtime.utils.git_handler import CommandResult, GitHandler
from wsaicode.security import SecurityAnalyzer, options
from wsaicode.storage.locations import get_conversation_dir
from wsaicode.utils.async_utils import (
    GENERAL_TIMEOUT,
...
class Runtime(FileEditRuntimeMixin):
    ...
    async def clone_or_init_repo(
        ...
        wsaicode_workspace_branch = f'wsaicode-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {wsaicode_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .wsaicode/setup.sh if it exists in the workspace or repository."""
    ...
    def maybe_setup_git_hooks(self):
        """Set up git hooks if .wsaicode/pre-commit.sh exists in the workspace or repository."""
        pre_commit_script = '.wsaicode/pre-commit.sh'
        ...
        if not isinstance(read_obs, ErrorObservation):
            # If the existing hook wasn't created by WSAI CODE, preserve it
            if 'This hook was installed by WSAI CODE' not in read_obs.content:
...
# It calls the pre-commit script in the .wsaicode directory
...
"""
    ...
    def _is_gitlab_repository(self, repo_name: str) -> bool:
        ...
        try:
            ...
            # This is a safe fallback since we'll just use the default .wsaicode
            return False
    ...
    def get_microagents_from_org_or_user(
        ...
        github.com/acme-co/.wsaicode exists. If it does, it will clone it and load
        the microagents from the ./microagents/ folder.

        For GitLab repositories, it will use wsaicode-config instead of .wsaicode
        since GitLab doesn't support repository names starting with non-alphanumeric
        characters.

        For Azure DevOps repositories, it will use org/wsaicode-config/wsaicode-config
        ...
        # For GitLab and Azure DevOps, use wsaicode-config (since .wsaicode is not a valid repo name)
        # For other providers, use .wsaicode
        if is_gitlab:
            org_wsaicode_repo = f'{org_name}/wsaicode-config'
        elif is_azure_devops:
            # Azure DevOps format: org/project/repo
            # For org-level config, use: org/wsaicode-config/wsaicode-config
            org_wsaicode_repo = f'{org_name}/wsaicode-config/wsaicode-config'
        else:
            org_wsaicode_repo = f'{org_name}/.wsaicode'

        self.log(
            'info',
            f'Checking for org-level microagents at {org_wsaicode_repo}',
        )

        # Try to clone the org-level repo
        try:
            # Create a temporary directory for the org-level repo
            org_repo_dir = self.workspace_root / f'org_wsaicode_{org_name}'
            self.log(
                'debug',
            ...
            try:
                ...
                self.log(
                    'debug',
                    f'org-level microagent directory {org_wsaicode_repo} not found: {str(e)}',
                )
                raise
            except Exception as e:
                self.log(
                    'debug',
                    f'Failed to get authenticated URL for {org_wsaicode_repo}: {str(e)}',
                )
                raise
            ...
            if isinstance(obs, CmdOutputObservation) and obs.exit_code == 0:
                self.log(
                    'info',
                    f'Successfully cloned org-level microagents from {org_wsaicode_repo}',
                )
                ...
                self.log(
                    'info',
                    f'Loaded {len(loaded_microagents)} microagents from org-level repository {org_wsaicode_repo}',
                )
                ...
                self.log(
                    'info',
                    f'No org-level microagents found at {org_wsaicode_repo} (exit_code: {exit_code})',
                )
                self.log(
                    'debug',
            ...
            self.log(
                'debug',
                f'Error loading org-level microagents from {org_wsaicode_repo}: {str(e)}',
            )
    ...
    def get_microagents_from_selected_repo(
        ...
        github.com/acme-co/.wsaicode and load microagents from there if it exists.
        ...
        """
        loaded_microagents: list[BaseMicroagent] = []
        microagents_dir = self.workspace_root / '.wsaicode' / 'microagents'
        repo_root = None
        ...
        if selected_repository:
            ...
            microagents_dir = repo_root / '.wsaicode' / 'microagents'
        ...
        # Check for legacy .wsaicode_instructions file
        obs = self.read(
            FileReadAction(path=str(self.workspace_root / '.wsaicode_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.wsaicode_instructions not present, trying to load from repository microagents_dir={microagents_dir}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.wsaicode_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'wsaicode_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroagent.load(
                    path='.wsaicode_instructions',
                    microagent_dir=None,
    ...
    @classmethod
    def setup(cls, config: WSAICodeConfig, headless_mode: bool = False):
        """Set up the environment for runtimes to be created."""

    @classmethod
    def teardown(cls, config: WSAICodeConfig):
