...
import wsaicode
from wsaicode.core.exceptions import AgentRuntimeBuildError
from wsaicode.core.logger import wsaicode_logger as logger
from wsaicode.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def get_runtime_image_repo() -> str:
    return os.getenv('OH_RUNTIME_RUNTIME_IMAGE_REPO', 'ghcr.io/wsaicode/runtime')
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the WSAI CODE runtime Docker image using the docker build folder.
    ...
    See https://docs.wsai-code.dev/usage/architecture/runtime for more details.
    """
...
def prep_build_folder(
    ...
    wsaicode_source_dir = Path(wsaicode.__file__).parent
    project_root = wsaicode_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'wsaicode' directory (Source code)
    shutil.copytree(
        wsaicode_source_dir,
        Path(build_folder, 'code', 'wsaicode'),
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(wsaicode_source_dir, file)
        if not src.exists():
...
def get_hash_for_lock_files(base_image: str, enable_browser: bool = True) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(wsaicode_source_dir, file)
        if not src.exists():
            src = Path(wsaicode_source_dir.parent, file)
