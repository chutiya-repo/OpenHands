...
from wsaicode.runtime.utils.system import check_port_available
...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str, runtime_id: str | None = None) -> None:
        ...
        if username not in filter(None, [RUNTIME_USERNAME, 'root', 'wsaicode']):
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or wsaicode user. '
        ...
        if explicit_base:
            ...
            cmd = (
                ...
                + f'sudo chown -R {username}:{username} /wsaicode/.openvscode-server\n'
                + f'cd {workspace_path}\n'
                + 'exec /wsaicode/.openvscode-server/bin/openvscode-server '
