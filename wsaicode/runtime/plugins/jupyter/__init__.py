...
from wsaicode.events.action import Action, IPythonRunCellAction
from wsaicode.events.observation import IPythonRunCellObservation
from wsaicode.runtime.plugins.jupyter.execute_server import JupyterKernel
...
class JupyterPlugin(Plugin):
    ...
    async def initialize(
        self, username: str, kernel_id: str = 'wsaicode-default'
    ) -> None:
        ...
        if not is_local_runtime:
            ...
            poetry_prefix = (
                'cd /wsaicode/code\n'
                'export POETRY_VIRTUALENVS_PATH=/wsaicode/poetry;\n'
                'export PYTHONPATH=/wsaicode/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/wsaicode/micromamba;\n'
                '/wsaicode/micromamba/bin/micromamba run -n wsaicode '
            )
        else:
            # LocalRuntime
            prefix = ''
            code_repo_path = os.environ.get('WSAI_CODE_REPO_PATH')
            if not code_repo_path:
                raise ValueError(
                    'WSAI_CODE_REPO_PATH environment variable is not set. '
