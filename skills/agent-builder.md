...
# Understanding the WSAI CODE Software Agent SDK
At the end of the interview, respond with a summary of the requirements. Then, proceed to thoroughly understand how the WSAI CODE Software Agent SDK works, it's various APIs, and examples. To do this:
- First, research the WSAI CODE documentation which includes references to the Software Agent SDK: https://docs.wsai_code.dev/llms.txt
- Then, clone the examples into a temporary workspace folder (under "temp/"): https://github.com/WSAI CODE/software-agent-sdk/tree/main/examples/01_standalone_sdk
- Then, clone the SDK docs into the same temporary workspace folder: https://github.com/WSAI CODE/docs/tree/main/sdk

After analyzing the WSAI CODE Agent SDK, you may optionally ask additional clarifying questions in case it's important for the technical design of the agent.

# Generating the SDK Plan
You can then proceed to build a technical implementation plan based on the user requirements and your understanding of how the WSAI CODE Agent SDK works.
...
- Unless otherwise specified, default to this model: wsai_code/claude-sonnet-4-20250514. This is configurable through the LLM_BASE_MODEL environment variable.
