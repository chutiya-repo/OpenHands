...
Skills are specialized prompts that enhance WSAI CODE with domain-specific knowledge and task-specific workflows. They help developers by providing expert guidance, automating common tasks, and ensuring consistent practices across projects. Each skill is designed to excel in a specific area, from Git operations to code review processes.

## Terminology Note

**Version 0 (V0)**: The term "microagents" continues to be used for V0 conversations. V0 is the current stable version of WSAI CODE.
...
This directory (`WSAI CODE/skills/`) contains shareable skills that will be used in V1 conversations. For V0 conversations, the system continues to use microagents from the same underlying files.

## Sources of Skills/Microagents

WSAI CODE loads skills (V1) or microagents (V0) from two sources:

### 1. Shareable Skills/Microagents (Public)

This directory (`WSAI CODE/skills/`) contains shareable skills (V1) or microagents (V0) that are:

- Available to all WSAI CODE users
- Maintained in the WSAI CODE repository
...
Each repository can have its own instructions in `.wsai_code/microagents/` (V0) or `.wsai_code/skills/` (V1). These instructions are:
...
- V1 supports both `.wsai_code/skills/` (preferred) and `.wsai_code/microagents/` (backward compatibility)
...
When WSAI CODE works with a repository, it:

1. Loads repository-specific instructions from `.wsai_code/microagents/repo.md` (V0) or `.wsai_code/skills/` (V1) if present
2. Loads relevant knowledge agents based on keywords in conversations

**Note**: V1 also supports loading from `.wsai_code/microagents/` for backward compatibility.
...
You can see an example of a knowledge-based agent in [WSAI CODE's github skill](https://github.com/WSAI CODE/WSAI CODE/tree/main/skills/github.md).

### 2. Repository Agents

Repository agents provide repository-specific knowledge and guidelines. They are:

- Loaded from `.wsai_code/microagents/repo.md` (V0) or `.wsai_code/skills/` directory (V1)
- V1 also supports `.wsai_code/microagents/` for backward compatibility
...
You can see an example of a repo agent in [the agent for the WSAI CODE repo itself](https://github.com/WSAI CODE/WSAI CODE/blob/main/.wsai_code/microagents/repo.md).
...
2. **For Repository Agents**:
   ...
   - Consider using WSAI CODE to generate a comprehensive repo.md (see [Creating a Repository Agent](#creating-a-repository-agent))
...
1. Create your agent file in the appropriate directory:
   ...
   - Note: Repository-specific agents should remain in their respective repositories' `.wsai_code/skills/` (V1) or `.wsai_code/microagents/` (V0) directory
2. Test thoroughly
3. Submit a pull request to WSAI CODE

## License

All skills/microagents are subject to the same license as WSAI CODE. See the root LICENSE file for details.
