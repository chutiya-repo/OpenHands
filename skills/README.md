...
Each repository can have its own instructions in `.wsaicode/microagents/` (V0) or `.wsaicode/skills/` (V1). These instructions are:
...
- V1 supports both `.wsaicode/skills/` (preferred) and `.wsaicode/microagents/` (backward compatibility)
...
1. Loads repository-specific instructions from `.wsaicode/microagents/repo.md` (V0) or `.wsaicode/skills/` (V1) if present
2. Loads relevant knowledge agents based on keywords in conversations

**Note**: V1 also supports loading from `.wsaicode/microagents/` for backward compatibility.
...
You can see an example of a knowledge-based agent in [WSAI CODE's github skill](https://github.com/wsaicode/wsaicode/tree/main/skills/github.md).

### 2. Repository Agents

Repository agents provide repository-specific knowledge and guidelines. They are:

- Loaded from `.wsaicode/microagents/repo.md` (V0) or `.wsaicode/skills/` directory (V1)
- V1 also supports `.wsaicode/microagents/` for backward compatibility
...
You can see an example of a repo agent in [the agent for the WSAI CODE repo itself](https://github.com/wsaicode/wsaicode/blob/main/.wsaicode/microagents/repo.md).
...
1. Create your agent file in the appropriate directory:
   ...
   - Note: Repository-specific agents should remain in their respective repositories' `.wsaicode/skills/` (V1) or `.wsaicode/microagents/` (V0) directory
2. Test thoroughly
