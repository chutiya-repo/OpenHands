...
```bash
# npm
npm install @wsaicode/ui

# yarn
yarn add @wsaicode/ui

# pnpm
pnpm add @wsaicode/ui
...
```tsx
import { Button, Typography } from "@wsaicode/ui";
import "@wsaicode/ui/styles";
...
# Replace `path/to/wsaicode-ui-x.x.x.tgz` with the actual path to the generated `.tgz` file.
npm install path/to/wsaicode-ui-x.x.x.tgz
```
