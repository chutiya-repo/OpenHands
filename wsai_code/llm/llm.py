...
from wsai_code.llm.model_features import get_features
...
from wsai_code.core.exceptions import LLMNoResponseError
from wsai_code.core.logger import wsai_code_logger as logger
from wsai_code.core.message import Message
from wsai_code.llm.debug_mixin import DebugMixin
from wsai_code.llm.fn_call_converter import (
    STOP_WORDS,
    convert_fncall_messages_to_non_fncall_messages,
    convert_non_fncall_messages_to_fncall_messages,
)
from wsai_code.llm.retry_mixin import RetryMixin
...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        if self.config.model.startswith('wsai_code/'):
            model_name = self.config.model.removeprefix('wsai_code/')
            self.config.model = f'litellm_proxy/{model_name}'
            self.config.base_url = 'https://llm-proxy.app.all-hands.dev/'
            logger.debug(
                f'Rewrote wsai_code/{model_name} to {self.config.model} with base URL {self.config.base_url}'
            )
        ...
        # Azure issue: https://github.com/WSAI CODE/WSAI CODE/issues/6777
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if mock_function_calling and 'tools' in kwargs:
                ...
                if (
                    'wsai_code-lm' in self.config.model
                ...
                if 'wsai_code-lm' in self.config.model:
                    # If we don't have this, we might run into issue when serving wsai_code-lm
                    # using SGLang
