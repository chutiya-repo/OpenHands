...
from wsai_code.events.nested_event_store import NestedEventStore
...
from wsai_code.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderHandler
...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def get_running_agent_loops(
        ...
        conversation_ids = {
            name[len('wsai_code-runtime-') :]
            for name in names
            if name.startswith('wsai_code-runtime-')
        }
    ...
    async def close_session(self, sid: str):
        ...
        try:
            container = self.docker_client.containers.get(f'wsai_code-runtime-{sid}')
    ...
    async def get_agent_loop_info(
        ...
        for container in containers:
            if not container.name or not container.name.startswith(
                ...
                continue
            conversation_id = container.name[len('wsai_code-runtime-') :]
    ...
    def get_nested_url_for_container(self, container: Container) -> str:
        ...
        conversation_id = container_name[len('wsai_code-runtime-') :]
    ...
    async def _create_runtime(
        ...
        env_vars['USER'] = 'wsai_code' if config.run_as_wsai_code else 'root'
        ...
        if config.file_store == 'local':
            ...
            volumes.append(
                f'{file_store_path}/{conversation_dir}:/root/.wsai_code/{conversation_dir}:rw'
            )
