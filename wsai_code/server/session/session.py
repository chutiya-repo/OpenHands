...
from wsai_code.core.config.mcp_config import WSAI CODEMCPConfigImpl
from wsai_code.core.exceptions import MicroagentValidationError
from wsai_code.core.logger import WSAI CODELoggerAdapter
from wsai_code.core.schema import AgentState
from wsai_code.events.action import MessageAction, NullAction
from wsai_code.events.event import Event, EventSource
from wsai_code.events.observation import (
    AgentStateChangedObservation,
    CmdOutputObservation,
    NullObservation,
)
from wsai_code.events.observation.agent import RecallObservation
...
class WebSession:
    ...
    (see wsai_code.server.session.__init__) so downstream imports/tests continue to
    ...
    Attributes:
        ...
        config: Effective WSAI CODE configuration for this conversation.
    ...
    def __init__(
        ...
        self.logger = WSAI CODELoggerAdapter(extra={'session_id': sid})
    ...
    async def initialize_agent(
        ...
        # Add WSAI CODE' MCP server by default
        wsai_code_mcp_server, wsai_code_mcp_stdio_servers = (
            WSAI CODEMCPConfigImpl.create_default_mcp_server_config(
                self.config.mcp_host, self.config, self.user_id
            )
        )

        if wsai_code_mcp_server:
            self.config.mcp.shttp_servers.append(wsai_code_mcp_server)
            self.logger.debug('Added default MCP HTTP server to config')

            self.config.mcp.stdio_servers.extend(wsai_code_mcp_stdio_servers)
...
# wsai_code.server.session.session import Session
Session = WebSession
