...
from wsai_code.events.action import Action, IPythonRunCellAction
from wsai_code.events.observation import IPythonRunCellObservation
from wsai_code.runtime.plugins.jupyter.execute_server import JupyterKernel
...
class JupyterPlugin(Plugin):
    ...
    async def initialize(
        self, username: str, kernel_id: str = 'wsai_code-default'
    ) -> None:
        ...
        if not is_local_runtime:
            ...
            poetry_prefix = (
                'cd /wsai_code/code\n'
                'export POETRY_VIRTUALENVS_PATH=/wsai_code/poetry;\n'
                'export PYTHONPATH=/wsai_code/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/wsai_code/micromamba;\n'
                '/wsai_code/micromamba/bin/micromamba run -n wsai_code '
            )
        else:
            # LocalRuntime
            prefix = ''
