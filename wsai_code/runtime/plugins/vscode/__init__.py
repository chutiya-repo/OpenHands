...
from wsai_code.runtime.utils.system import check_port_available
...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str, runtime_id: str | None = None) -> None:
        ...
        if username not in filter(None, [RUNTIME_USERNAME, 'root', 'wsai_code']):
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or wsai_code user. '
        ...
        if explicit_base:
            ...
            cmd = (
                ...
                + f'sudo chown -R {username}:{username} /wsai_code/.openvscode-server\n'
                + f'cd {workspace_path}\n'
                + 'exec /wsai_code/.openvscode-server/bin/openvscode-server '
