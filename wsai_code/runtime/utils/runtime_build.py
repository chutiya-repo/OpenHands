...
import wsai_code
from wsai_code.core.exceptions import AgentRuntimeBuildError
from wsai_code.core.logger import wsai_code_logger as logger
from wsai_code.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def get_runtime_image_repo() -> str:
    return os.getenv('OH_RUNTIME_RUNTIME_IMAGE_REPO', 'ghcr.io/wsai_code/runtime')
...
def prep_build_folder(
    ...
    wsai_code_source_dir = Path(wsai_code.__file__).parent
    project_root = wsai_code_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'wsai_code' directory (Source code)
    shutil.copytree(
        wsai_code_source_dir,
        Path(build_folder, 'code', 'wsai_code'),
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(wsai_code_source_dir, file)
        if not src.exists():
...
def get_hash_for_lock_files(base_image: str, enable_browser: bool = True) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(wsai_code_source_dir, file)
        if not src.exists():
            src = Path(wsai_code_source_dir.parent, file)
