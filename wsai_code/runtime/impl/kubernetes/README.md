# WSAI CODE Kubernetes Runtime

This directory contains the Kubernetes runtime implementation for WSAI CODE, which allows the software to run on Kubernetes clusters for scalable and isolated execution environments.

## Local Development with KIND

For local development and testing, WSAI CODE provides a convenient setup using KIND (Kubernetes IN Docker) that creates a local Kubernetes cluster.
...
To use the Kubernetes runtime, you need to configure WSAI CODE properly. The configuration is done through a TOML configuration file.
...
2. **Runtime Container Image**: Specify the container image to use for the runtime environment
   ```toml
   [sandbox]
   runtime_container_image = "docker.wsai_code.dev/wsai_code/runtime:1.1-nikolaik"
   ```

#### Additional Kubernetes Options

WSAI CODE provides extensive configuration options for Kubernetes deployments under the `[kubernetes]` section. These options allow you to customize:
...
To set up and run WSAI CODE with the Kubernetes runtime locally:
...
2. Automatically start the WSAI CODE application using mirrord
