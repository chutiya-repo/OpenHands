...
from wsai_code.core.exceptions import AgentRuntimeDisconnectedError
...
from wsai_code.events.serialization import event_to_dict, observation_from_dict
...
from wsai_code.runtime.impl.docker.docker_runtime import (
    APP_PORT_RANGE_1,
    APP_PORT_RANGE_2,
...
def check_dependencies(code_repo_path: str, check_browser: bool) -> None:
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/WSAI CODE/WSAI CODE/blob/main/Development.md to install WSAI CODE.'
...
class LocalRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        logger.warning(
            ...
            'This is an experimental feature, please report issues to https://github.com/WSAI CODE/WSAI CODE/issues. '
            '`run_as_wsai_code` will be ignored since the current user will be used to launch the server. '
        ...
        if self.sid in _RUNNING_SERVERS:
            ...
            if self.config.workspace_base is not None:
                ...
                self._temp_workspace = tempfile.mkdtemp(
                    prefix=f'wsai_code_workspace_{self.sid}',
                )
    ...
    def setup(cls, config: WSAI CODEConfig, headless_mode: bool = False):
        ...
        if should_check_dependencies:
            code_repo_path = os.path.dirname(os.path.dirname(wsai_code.__file__))
...
def _create_server(
    ...
    temp_workspace = tempfile.mkdtemp(
        prefix=f'wsai_code_workspace_{workspace_prefix}',
    )
...
def _get_plugins(config: WSAI CODEConfig) -> list[PluginRequirement]:
