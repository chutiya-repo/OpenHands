...
from wsai_code.core.logger import RollingLogger
from wsai_code.core.logger import wsai_code_logger as logger
from wsai_code.runtime.builder.base import RuntimeBuilder
from wsai_code.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running wsai_code in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official wsai_code app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside wsai_code-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
