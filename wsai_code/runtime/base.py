...
from wsai_code.core.config import WSAI CODEConfig, SandboxConfig
from wsai_code.core.config.mcp_config import MCPConfig, MCPStdioServerConfig
...
from wsai_code.events.serialization.action import ACTION_TYPE_TO_CLASS
...
from wsai_code.microagent import (
    BaseMicroagent,
    load_microagents_from_dir,
)
from wsai_code.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from wsai_code.runtime.runtime_status import RuntimeStatus
from wsai_code.runtime.utils.edit import FileEditRuntimeMixin
from wsai_code.runtime.utils.git_handler import CommandResult, GitHandler
from wsai_code.security import SecurityAnalyzer, options
from wsai_code.storage.locations import get_conversation_dir
from wsai_code.utils.async_utils import (
    GENERAL_TIMEOUT,
...
class Runtime(FileEditRuntimeMixin):
    ...
    async def clone_or_init_repo(
        ...
        wsai_code_workspace_branch = f'wsai_code-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {wsai_code_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .wsai_code/setup.sh if it exists in the workspace or repository."""
        setup_script = '.wsai_code/setup.sh'
    ...
    def maybe_setup_git_hooks(self):
        """Set up git hooks if .wsai_code/pre-commit.sh exists in the workspace or repository."""
        pre_commit_script = '.wsai_code/pre-commit.sh'
...
"""
    ...
    def _is_gitlab_repository(self, repo_name: str) -> bool:
        ...
        try:
            ...
            # This is a safe fallback since we'll just use the default .wsai_code
            return False
    ...
    def get_microagents_from_org_or_user(
        ...
        github.com/acme-co/.wsai_code exists. If it does, it will clone it and load
        the microagents from the ./microagents/ folder.

        For GitLab repositories, it will use wsai_code-config instead of .wsai_code
        since GitLab doesn't support repository names starting with non-alphanumeric
        characters.

        For Azure DevOps repositories, it will use org/wsai_code-config/wsai_code-config
        ...
        # For GitLab and Azure DevOps, use wsai_code-config (since .wsai_code is not a valid repo name)
        # For other providers, use .wsai_code
        if is_gitlab:
            org_wsai_code_repo = f'{org_name}/wsai_code-config'
        elif is_azure_devops:
            # Azure DevOps format: org/project/repo
            # For org-level config, use: org/wsai_code-config/wsai_code-config
            org_wsai_code_repo = f'{org_name}/wsai_code-config/wsai_code-config'
        else:
            org_wsai_code_repo = f'{org_name}/.wsai_code'

        self.log(
            'info',
            f'Checking for org-level microagents at {org_wsai_code_repo}',
        )

        # Try to clone the org-level repo
        try:
            # Create a temporary directory for the org-level repo
            org_repo_dir = self.workspace_root / f'org_wsai_code_{org_name}'
            self.log(
                'debug',
            ...
            try:
                ...
                self.log(
                    'debug',
                    f'org-level microagent directory {org_wsai_code_repo} not found: {str(e)}',
                )
                raise
            except Exception as e:
                self.log(
                    'debug',
                    f'Failed to get authenticated URL for {org_wsai_code_repo}: {str(e)}',
                )
                raise
            ...
            if isinstance(obs, CmdOutputObservation) and obs.exit_code == 0:
                self.log(
                    'info',
                    f'Successfully cloned org-level microagents from {org_wsai_code_repo}',
                )
                ...
                self.log(
                    'info',
                    f'Loaded {len(loaded_microagents)} microagents from org-level repository {org_wsai_code_repo}',
                )
                ...
                self.log(
                    'info',
                    f'No org-level microagents found at {org_wsai_code_repo} (exit_code: {exit_code})',
                )
                self.log(
                    'debug',
            ...
            self.log(
                'debug',
                f'Error loading org-level microagents from {org_wsai_code_repo}: {str(e)}',
            )
    ...
    def get_microagents_from_selected_repo(
        ...
        github.com/acme-co/.wsai_code and load microagents from there if it exists.
        ...
        """
        loaded_microagents: list[BaseMicroagent] = []
        microagents_dir = self.workspace_root / '.wsai_code' / 'microagents'
        repo_root = None
        ...
        if selected_repository:
            ...
            microagents_dir = repo_root / '.wsai_code' / 'microagents'
        ...
        # Check for legacy .wsai_code_instructions file
        obs = self.read(
            FileReadAction(path=str(self.workspace_root / '.wsai_code_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.wsai_code_instructions not present, trying to load from repository microagents_dir={microagents_dir}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.wsai_code_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'wsai_code_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroagent.load(
                    path='.wsai_code_instructions',
                    microagent_dir=None,
    ...
    @classmethod
    def setup(cls, config: WSAI CODEConfig, headless_mode: bool = False):
        """Set up the environment for runtimes to be created."""

    @classmethod
    def teardown(cls, config: WSAI CODEConfig):
