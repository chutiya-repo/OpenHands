...
The WSAI CODE Runtime folder contains the core components responsible for executing actions and managing the runtime environment for the WSAI CODE project. This README provides an overview of the main components and their interactions.
You can learn more about how the runtime works in the [Docker Runtime](https://docs.wsai_code.dev/usage/architecture/runtime) documentation.
...
6. **Sandbox Environment**:
   ...
   - Actions received from the WSAI CODE backend are executed in this sandboxed environment.
...
This is the default runtime used within WSAI CODE.
...
**Important: This runtime provides no isolation as it runs directly on the host machine. All actions are executed with the same permissions as the user running WSAI CODE. For secure execution with proper isolation, use the Docker Runtime instead.**
...
At the time of this writing, this is mostly used in parallel evaluation, such as this example for [SWE-Bench](https://github.com/WSAI CODE/WSAI CODE/tree/main/evaluation/benchmarks/swe_bench#run-inference-on-remoteruntime-experimental).

## Related Components

- The runtime interacts closely with the event system defined in the `wsai_code.events` module.
- It relies on configuration classes from `wsai_code.core.config`.
- Logging is handled through `wsai_code.core.logger`.

This section provides an overview of the WSAI CODE Runtime folder. For more detailed information on specific components or usage, please refer to the individual files and their docstrings.
