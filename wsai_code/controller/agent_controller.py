...
from wsai_code.controller.state.state_tracker import StateTracker
...
from wsai_code.events.serialization.event import truncate_content
...
class AgentController:
    ...
    async def start_delegate(self, action: AgentDelegateAction) -> None:
        ...
        WSAI CODE is a multi-agentic system. A `task` is a conversation between
        WSAI CODE (the whole system) and the user, which might involve one or more inputs
        ...
        if action.runnable:
            if self.state.confirmation_mode and (
                ...
                if self.agent.config.cli_mode:
                    ...
                    # See issue: https://github.com/WSAI CODE/WSAI CODE/issues/10464
