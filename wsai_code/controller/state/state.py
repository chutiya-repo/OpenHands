...
from wsai_code.events.action.agent import AgentFinishAction
...
from wsai_code.storage.locations import get_conversation_agent_state_filename
...
@dataclass
class State:
    """Represents the running state of an agent in the WSAI CODE system, saving data of its operation and memory.
    ...
    def to_llm_metadata(self, model_name: str, agent_name: str) -> dict:
        metadata = {
            ...
            'trace_version': wsai_code.__version__,
            'trace_user_id': self.user_id,
            'tags': [
                f'model:{model_name}',
                f'agent:{agent_name}',
                f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
                f'wsai_code_version:{wsai_code.__version__}',
            ],
        }
        return metadata
