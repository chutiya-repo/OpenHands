"""Database configuration and session management for WSAI CODE App Server."""
...
from wsai_code.app_server.services.injector import Injector, InjectorState
...
class DbSessionInjector(BaseModel, Injector[async_sessionmaker]):
    ...
    def fill_empty_fields(self):
        ...
        if self.name is None:
            self.name = os.getenv('DB_NAME', 'wsai_code')
    ...
    async def get_async_db_engine(self) -> AsyncEngine:
        ...
        if self.gcp_db_instance:  # GCP environments
            ...
            if self.host:
                ...
                url = f'sqlite+aiosqlite:///{str(self.persistence_dir)}/wsai_code.db'
    ...
    def get_db_engine(self) -> Engine:
        ...
        if self.gcp_db_instance:  # GCP environments
            ...
            if self.host:
                ...
                url = f'sqlite:///{self.persistence_dir}/wsai_code.db'
            engine = create_engine(
                url,
                pool_size=self.pool_size,
                max_overflow=self.max_overflow,
