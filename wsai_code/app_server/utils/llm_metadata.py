"""Utility functions for LLM metadata in WSAI CODE V1 conversations."""
...
def should_set_litellm_extra_body(model_name: str) -> bool:
    ...
    Only set litellm_extra_body for wsai_code models to avoid issues
    with providers that don't support extra_body parameters.

    The SDK internally translates "wsai_code/" prefix to "litellm_proxy/"
    ...
    """
    return 'wsai_code/' in model_name or 'litellm_proxy/' in model_name
...
def get_llm_metadata(
    ...
    """Generate LLM metadata for WSAI CODE V1 conversations.
    ...
    """
    wsai_code_version = wsai_code.__version__

    metadata: dict[str, Any] = {
        'trace_version': wsai_code_version,
        'tags': [
            'app:wsai_code',
            f'model:{model_name}',
            f'type:{llm_type}',
            f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
            f'wsai_code_version:{wsai_code_version}',
