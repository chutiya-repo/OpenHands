...
from wsai_code.sdk.context.agent_context import AgentContext
from wsai_code.sdk.context.condenser import LLMSummarizingCondenser
from wsai_code.sdk.context.skills import load_user_skills
from wsai_code.sdk.llm import LLM
from wsai_code.sdk.security.analyzer import SecurityAnalyzerBase
from wsai_code.sdk.security.confirmation_policy import (
    AlwaysConfirm,
    ConfirmationPolicyBase,
    ConfirmRisky,
    NeverConfirm,
)
from wsai_code.sdk.security.llm_analyzer import LLMSecurityAnalyzer
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        try:
            ...
            # Load user skills from ~/.wsai_code/skills/ directory
            # Uses the SDK's load_user_skills() function which handles loading from
            # ~/.wsai_code/skills/ and ~/.wsai_code/microagents/ (for backward compatibility)
            try:
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            checkout_command = f'git checkout -b {wsai_code_workspace_branch}'
    ...
    async def maybe_run_setup_script(
        ...
        setup_script = workspace.working_dir + '/.wsai_code/setup.sh'
    ...
    async def maybe_setup_git_hooks(
        ...
        command = 'mkdir -p .git/hooks && chmod +x .wsai_code/pre-commit.sh'
