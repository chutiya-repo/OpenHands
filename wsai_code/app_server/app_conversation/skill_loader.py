...
- Global skills from WSAI CODE/skills/
- User skills from ~/.wsai_code/skills/
...
import wsai_code
from wsai_code.app_server.sandbox.sandbox_models import SandboxInfo
from wsai_code.app_server.user.user_context import UserContext
...
from wsai_code.sdk.context.skills import Skill
from wsai_code.sdk.workspace.remote.async_remote_workspace import AsyncRemoteWorkspace

_logger = logging.getLogger(__name__)

# Path to global skills directory
GLOBAL_SKILLS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(wsai_code.__file__)),
    'skills',
)
...
def load_global_skills() -> list[Skill]:
    """Load global skills from WSAI CODE/skills/ directory.
...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.wsai_code', 'owner')
        - GitLab: ('owner/wsai_code-config', 'owner')
        - Azure DevOps: ('org/wsai_code-config/wsai_code-config', 'org')
    """
    ...
    return org_wsai_code_repo, org_name
...
def _merge_repo_skills_with_precedence(
    ...
    2. .wsai_code/skills/ directory
    3. .wsai_code/microagents/ directory (backward compatibility)

    Args:
        special_skills: Skills from special files in repo root
        skills_dir_skills: Skills from .wsai_code/skills/ directory
        microagents_dir_skills: Skills from .wsai_code/microagents/ directory
...
async def load_repo_skills(
    ...
    2. .md files in .wsai_code/skills/ directory (preferred)
    3. .md files in .wsai_code/microagents/ directory (for backward compatibility)
    ...
    try:
        ...
        # Load .md files from .wsai_code/skills/ directory (preferred)
        skills_dir = f'{repo_root}/.wsai_code/skills'
        skills_dir_skills = await _find_and_load_skill_md_files(
            workspace, skills_dir, working_dir
        )

        # Load .md files from .wsai_code/microagents/ directory (backward compatibility)
        microagents_dir = f'{repo_root}/.wsai_code/microagents'
...
async def _get_org_repository_url(
    org_wsai_code_repo: str, user_context: UserContext
) -> str | None:
    """Get authenticated Git URL for organization repository.

    Args:
        org_wsai_code_repo: Organization repository path
    ...
    """
    try:
        remote_url = await user_context.get_authenticated_git_url(org_wsai_code_repo)
        return remote_url
    except AuthenticationError as e:
        _logger.debug(
            f'org-level skill directory {org_wsai_code_repo} not found: {str(e)}'
        )
        return None
    except Exception as e:
        _logger.debug(
...
async def _clone_org_repository(
    ...
    Args:
        ...
        working_dir: Working directory for command execution
        org_wsai_code_repo: Organization repository path (for logging)
    ...
    if result.exit_code != 0:
        _logger.info(
            f'No org-level skills found at {org_wsai_code_repo} (exit_code: {result.exit_code})'
        )
        _logger.debug(f'Clone command output: {result.stderr}')
        return False

    _logger.info(f'Successfully cloned org-level skills from {org_wsai_code_repo}')
    return True
...
async def load_org_skills(
    ...
    try:
        ...
        org_wsai_code_repo, org_name = await _determine_org_repo_path(
            selected_repository, user_context
        )

        _logger.info(f'Checking for org-level skills at {org_wsai_code_repo}')

        # Get authenticated URL for org repository
        remote_url = await _get_org_repository_url(org_wsai_code_repo, user_context)
        if not remote_url:
            return []

        # Clone the organization repository
        org_repo_dir = f'{working_dir}/_org_wsai_code_{org_name}'
        clone_success = await _clone_org_repository(
            workspace, remote_url, org_repo_dir, working_dir, org_wsai_code_repo
        )
        ...
        _logger.info(
            f'Loaded {len(loaded_skills)} skills from org-level repository {org_wsai_code_repo}: {[s.name for s in loaded_skills]}'
        )
