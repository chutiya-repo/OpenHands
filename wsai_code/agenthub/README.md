...
For example, `wsai_code/agenthub/codeact_agent`, etc.
...
The `state` represents the running state of an agent in the WSAI CODE system. The class handles saving and restoring the agent session. It is serialized in a pickle.
...
WSAI CODE is a multi-agentic system. Agents can delegate tasks to other agents, whether
...
A `task` is an end-to-end conversation between WSAI CODE (the whole system) and the user,
...
WSAI CODE repo has`. Let's assume the default agent is CodeActAgent.
