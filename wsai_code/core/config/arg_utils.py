...
# Tag: Legacy-V0
"""Centralized command line argument configuration for WSAI CODE CLI and headless modes."""
...
def get_cli_parser() -> argparse.ArgumentParser:
    ...
    description = (
        'Welcome to WSAI CODE: Code Less, Make More\n\n'
        'WSAI CODE supports two main commands:\n'
        '  serve - Launch the WSAI CODE GUI server (web interface)\n'
        '  cli   - Run WSAI CODE in CLI mode (terminal interface)\n\n'
        'Running "wsai_code" without a command is the same as "wsai_code cli"'
    )
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='For more information about a command, run: wsai_code COMMAND --help',
    )
    ...
    subparsers = parser.add_subparsers(
        dest='command',
        title='commands',
        description='WSAI CODE supports two main commands:',
        metavar='COMMAND',
    )

    # Add 'serve' subcommand
    serve_parser = subparsers.add_parser(
        'serve', help='Launch the WSAI CODE GUI server using Docker (web interface)'
    )
    ...
    cli_parser = subparsers.add_parser(
        'cli', help='Run WSAI CODE in CLI mode (terminal interface)'
    )
...
def get_evaluation_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description='Run WSAI CODE in evaluation mode')
    add_common_arguments(parser)
    add_headless_specific_arguments(parser)
