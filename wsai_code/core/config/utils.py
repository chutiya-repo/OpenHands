...
from wsai_code.core import logger
from wsai_code.core.config.agent_config import AgentConfig
from wsai_code.core.config.arg_utils import get_headless_parser
from wsai_code.core.config.condenser_config import (
    CondenserConfig,
    condenser_config_from_toml_section,
    create_condenser_config,
)
from wsai_code.core.config.extended_config import ExtendedConfig
from wsai_code.core.config.kubernetes_config import KubernetesConfig
...
def load_from_env(
    cfg: WSAI CODEConfig, env_or_toml_dict: dict | MutableMapping[str, str]
) -> None:
    ...
    Args:
        cfg: The WSAI CODEConfig object to set attributes on.
...
def load_from_toml(cfg: WSAI CODEConfig, toml_file: str = 'config.toml') -> None:
    """Load the config from the toml file. Supports both styles of config vars.

    Args:
        cfg: The WSAI CODEConfig object to update attributes of.
    ...
    # set LLMSummarizingCondenserConfig as default
        ...
        from wsai_code.core.config.condenser_config import LLMSummarizingCondenserConfig
    ...
    for key in toml_config:
        if key.lower() not in known_sections:
            logger.wsai_code_logger.warning(f'Unknown section [{key}] in {toml_file}')
...
def finalize_config(cfg: WSAI CODEConfig) -> None:
...
def get_agent_config_arg(
    ...
    logger.wsai_code_logger.debug(f'Loading agent config from {agent_config_arg}')
    ...
    try:
        ...
        logger.wsai_code_logger.info(f'Config file not found: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.wsai_code_logger.error(
    ...
    logger.wsai_code_logger.debug(f'Loading from toml failed for {agent_config_arg}')
    return None
...
def get_llm_config_arg(
    ...
    if not os.path.exists(toml_file):
        logger.wsai_code_logger.debug(f'Config file not found: {toml_file}')
        return None
...
def get_condenser_config_arg(
    ...
    try:
        ...
        logger.wsai_code_logger.info(f'Config file not found: {toml_file}. Error: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.wsai_code_logger.error(
...
def parse_arguments() -> argparse.Namespace:
    ...
    from wsai_code import get_version

    if args.version:
        print(f'WSAI CODE version: {get_version()}')
        sys.exit(0)

    return args


def register_custom_agents(config: WSAI CODEConfig) -> None:
...
def setup_config_from_args(args: argparse.Namespace) -> WSAI CODEConfig:
    ...
    3. ~/.wsai_code/settings.json and ~/.wsai_code/config.toml
    """
    # Load base config from toml and env vars
    config = load_wsai_code_config(config_file=args.config_file)

    # Override with command line arguments if provided
    if args.llm_config:
        logger.wsai_code_logger.debug(f'CLI specified LLM config: {args.llm_config}')
        ...
        if args.llm_config not in config.llms:
            ...
            if llm_config is None and args.config_file != os.path.join(
                os.path.expanduser('~'), '.wsai_code', 'config.toml'
            ):
