...
from wsai_code.core.config.config_utils import DEFAULT_WORKSPACE_MOUNT_PATH_IN_SANDBOX
...
def create_agent(config: WSAI CODEConfig, llm_registry: LLMRegistry) -> Agent:
...
def generate_sid(config: WSAI CODEConfig, session_name: str | None = None) -> str:
    """Generate a session id based on the session name and the jwt secret.

    The session ID is kept short to ensure Kubernetes resource names don't exceed
    the 63-character limit when prefixed with 'wsai_code-runtime-' (18 chars).
    ...
    # - 'wsai_code-runtime-' (18 chars) + session_id (32 chars) = 50 chars
