...
from wsai_code.controller.replay import ReplayManager
...
from wsai_code.core.loop import run_agent_until_done
from wsai_code.core.schema import AgentState
from wsai_code.core.setup import (
    create_agent,
    create_controller,
    create_memory,
    create_runtime,
...
from wsai_code.events import EventSource, EventStreamSubscriber
...
from wsai_code.events.observation import AgentStateChangedObservation
from wsai_code.io import read_input, read_task
from wsai_code.mcp import add_mcp_tools_to_agent
...
async def run_controller(
    ...
    It's only used when you launch wsai_code backend directly via cmdline.
    ...
    if agent.config.enable_mcp:
        ...
        _, wsai_code_mcp_stdio_servers = (
            WSAI CODEMCPConfigImpl.create_default_mcp_server_config(
                config.mcp_host, config, None
            )
        )
        runtime.config.mcp.stdio_servers.extend(wsai_code_mcp_stdio_servers)
...
if __name__ == '__main__':
    ...
    config: WSAI CODEConfig = setup_config_from_args(args)
