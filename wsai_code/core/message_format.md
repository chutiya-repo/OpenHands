# WSAI CODE Message Format and litellm Integration

## Overview

WSAI CODE uses its own `Message` class (`wsai_code/core/message.py`) which provides rich content support while maintaining compatibility with litellm's message handling system.

## Class Structure

Our `Message` class (`wsai_code/core/message.py`):
