...
def log_uncaught_exceptions(
    ...
    wsai_code_logger.error('Uncaught exception', exc_info=(ex_cls, ex, tb))


sys.excepthook = log_uncaught_exceptions
wsai_code_logger = logging.getLogger('wsai_code')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
wsai_code_logger.setLevel(current_log_level)

if DEBUG:
    wsai_code_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    wsai_code_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    wsai_code_logger.addHandler(json_log_handler(current_log_level))
    ...
    wsai_code_logger.addHandler(get_console_handler(current_log_level))

wsai_code_logger.addFilter(SensitiveDataFilter(wsai_code_logger.name))
wsai_code_logger.propagate = False
wsai_code_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of wsai_code/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    wsai_code_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    wsai_code_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        wsai_code_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class WSAI CODELoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = wsai_code_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
