...
from wsai_code.core.config import AgentConfig, WSAI CODEConfig, SandboxConfig
...
from wsai_code.events.stream import EventStreamSubscriber
...
from wsai_code.resolver.resolver_output import ResolverOutput
from wsai_code.resolver.utils import (
...
from wsai_code.utils.async_utils import GENERAL_TIMEOUT, call_async_from_sync
...
class IssueResolver:
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.app_config = self.update_wsai_code_config(
    ...
    def update_sandbox_config(
        cls,
        wsai_code_config: WSAI CODEConfig,
        base_container_image: str | None,
        runtime_container_image: str | None,
        is_experimental: bool,
    ) -> None:
        ...
        if (
            ...
            runtime_container_image = (
                f'ghcr.io/wsai_code/runtime:{wsai_code.__version__}-nikolaik'
            )
        ...
        wsai_code_config.sandbox.base_container_image = (
            sandbox_config.base_container_image
        )
        wsai_code_config.sandbox.runtime_container_image = (
            sandbox_config.runtime_container_image
        )
        wsai_code_config.sandbox.enable_auto_lint = sandbox_config.enable_auto_lint
        wsai_code_config.sandbox.use_host_network = sandbox_config.use_host_network
        wsai_code_config.sandbox.timeout = sandbox_config.timeout
        wsai_code_config.sandbox.local_runtime_url = sandbox_config.local_runtime_url
        wsai_code_config.sandbox.user_id = sandbox_config.user_id
    ...
    def initialize_runtime(
        ...
        logger.info('Checking for .wsai_code/setup.sh script...')
        runtime.maybe_run_setup_script()

        # Setup git hooks if they exist
        logger.info('Checking for .wsai_code/pre-commit.sh script...')
    ...
    async def resolve_issue(
        ...
        if self.repo_instruction is None:
            # Check for .wsai_code_instructions file in the workspace directory
            wsai_code_instructions_path = os.path.join(
                repo_dir, '.wsai_code_instructions'
            )
            if os.path.exists(wsai_code_instructions_path):
                with open(wsai_code_instructions_path, 'r') as f:
