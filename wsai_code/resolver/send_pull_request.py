...
from wsai_code.resolver.interfaces.forgejo import ForgejoIssueHandler
...
from wsai_code.resolver.io_utils import (
    load_single_resolver_output,
)
from wsai_code.resolver.patching import apply_diff, parse_patch
from wsai_code.resolver.resolver_output import ResolverOutput
from wsai_code.resolver.utils import identify_token
...
PR_SIGNATURE = (
    'Automatic fix generated by [WSAI CODE](https://github.com/WSAI CODE/WSAI CODE/) 🙌'
)
...
def make_commit(
    ...
    issue: Issue,
    issue_type: str,
    git_user_name: str = 'wsai_code',
    git_user_email: str = 'wsai_code@all-hands.dev',
) -> None:
    ...
    if not status_result.stdout.strip():
        ...
        raise RuntimeError('ERROR: WSAICode failed to make code changes.')
...
def send_pull_request(
    ...
    base_branch_name = f'wsai_code-fix-issue-{issue.number}'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'WSAI CODE made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = (
                'A new WSAI CODE update is available, but failed to parse or summarize '
                f'the changes:\n{additional_message}'
            )
