...
This directory contains various utility functions and classes used throughout WSAI CODE.

## Runtime Implementation Substitution

WSAI CODE provides an extensibility mechanism through the `get_impl` and `import_from` functions in `import_utils.py`. This mechanism allows applications built on WSAI CODE to customize behavior by providing their own implementations of WSAI CODE base classes.

### How It Works

1. Base classes define interfaces through abstract methods and properties
2. Default implementations are provided by WSAI CODE
3. Applications can provide custom implementations by:
   ...
   - Configuring WSAI CODE to use the custom implementation via configuration

### Example

```python
# In WSAI CODE base code:
class ConversationManager:
    @abstractmethod
    async def attach_to_conversation(self, sid: str) -> Conversation:
        """Attach to an existing conversation."""

# Default implementation in WSAI CODE:
...
WSAI CODE provides several components that can be extended:
...
1. `import_from(qual_name: str)`: Imports any Python value from its fully qualified name
   ```python
   UserAuth = import_from('wsai_code.server.user_auth.UserAuth')
   ```
