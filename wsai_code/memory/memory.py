...
from wsai_code.events.event import Event, EventSource, RecallType
from wsai_code.events.observation.agent import (
    MicroagentKnowledge,
    RecallObservation,
)
from wsai_code.events.observation.empty import NullObservation
from wsai_code.events.stream import EventStream, EventStreamSubscriber
...
USER_MICROAGENTS_DIR = Path.home() / '.wsai_code' / 'microagents'
...
class Memory:
    ...
    def __init__(
        ...
        # from typically WSAI CODE/skills (i.e., the PUBLIC microagents)
        self._load_global_microagents()

        # Load user microagents from ~/.wsai_code/microagents/
    ...
    def _load_user_microagents(self) -> None:
        """Loads microagents from the user's home directory (~/.wsai_code/microagents/)
