...
from wsai_code.microagent.types import MicroagentContentResponse, MicroagentResponse
...
class SuggestedTask(BaseModel):
    ...
    def get_prompt_for_task(
        ...
        env = Environment(
            loader=FileSystemLoader('wsai_code/integrations/templates/suggested_task')
        )
...
class BaseGitService(ABC):
    ...
    def _determine_microagents_path(self, repository_name: str) -> str:
        ...
        if actual_repo_name == '.wsai_code' or actual_repo_name == 'wsai_code-config':
            # For repository name ".wsai_code", scan "microagents" folder
            return 'microagents'
        else:
            # Default behavior: look for .wsai_code/microagents directory
            return '.wsai_code/microagents'
