...
from wsai_code.resolver.interfaces.issue import Issue, ReviewThread
from wsai_code.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from wsai_code.resolver.issue_resolver import (
    IssueResolver,
)
...
def mock_gitlab_token():
    ...
    """
    with patch(
        'wsai_code.resolver.issue_resolver.identify_token',
...
async def test_process_issue(
    ...
    with (
        patch('wsai_code.resolver.issue_resolver.create_runtime', mock_create_runtime),
        patch('wsai_code.resolver.issue_resolver.run_controller', mock_run_controller),
        patch.object(
        ...
        patch(
            'wsai_code.resolver.issue_resolver.SandboxConfig', return_value=MagicMock()
        ),
        patch(
            'wsai_code.resolver.issue_resolver.WSAI CODEConfig',
...
def test_file_instruction():
    ...
    # load prompt from wsai_code/resolver/prompts/resolve/basic.jinja
    with open('wsai_code/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()

    with open(
        'wsai_code/resolver/prompts/resolve/basic-conversation-instructions.jinja', 'r'
    ) as f:
...
def test_file_instruction_with_repo_instruction():
    ...
    # load repo instruction from wsai_code/resolver/prompts/repo_instructions/wsai-code-org___wsai_code-resolver.txt
    with open(
        'wsai_code/resolver/prompts/repo_instructions/wsai-code-org___wsai_code-resolver.txt',
        'r',
    ) as f:
...
This is a Python repo for wsai_code-resolver, a library that attempts to resolve github issues with the AI agent WSAI CODE.
