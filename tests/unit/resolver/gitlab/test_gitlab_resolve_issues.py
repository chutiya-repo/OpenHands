...
from wsaicode.resolver.interfaces.gitlab import GitlabIssueHandler, GitlabPRHandler
...
from wsaicode.resolver.issue_resolver import (
    IssueResolver,
)
...
def mock_gitlab_token():
    ...
    """
    with patch(
        'wsaicode.resolver.issue_resolver.identify_token',
...
async def test_process_issue(
    ...
    with (
        patch('wsaicode.resolver.issue_resolver.create_runtime', mock_create_runtime),
        patch('wsaicode.resolver.issue_resolver.run_controller', mock_run_controller),
        patch.object(
        ...
        patch(
            'wsaicode.resolver.issue_resolver.SandboxConfig', return_value=MagicMock()
        ),
        patch(
            'wsaicode.resolver.issue_resolver.WSAICodeConfig',
...
def test_file_instruction():
    ...
    # load prompt from wsaicode/resolver/prompts/resolve/basic.jinja
    with open('wsaicode/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()

    with open(
        'wsaicode/resolver/prompts/resolve/basic-conversation-instructions.jinja', 'r'
    ) as f:
...
def test_file_instruction_with_repo_instruction():
    ...
    # load repo instruction from wsaicode/resolver/prompts/repo_instructions/wsai-code-ai___wsaicode-resolver.txt
    with open(
        'wsaicode/resolver/prompts/repo_instructions/wsai-code-ai___wsaicode-resolver.txt',
        'r',
    ) as f:
...
This is a Python repo for wsaicode-resolver, a library that attempts to resolve github issues with the AI agent WSAI CODE.
