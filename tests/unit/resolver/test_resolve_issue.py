...
from wsai_code.resolver.issue_resolver import IssueResolver


def assert_sandbox_config(
    config: SandboxConfig,
    base_container_image=SandboxConfig.model_fields['base_container_image'].default,
    runtime_container_image='ghcr.io/wsai_code/runtime:mock-nikolaik',  # Default to mock version
...
def test_setup_sandbox_config_default():
    ...
    with mock.patch('wsai_code.__version__', 'mock'):
        wsai_code_config = WSAI CODEConfig()

        IssueResolver.update_sandbox_config(
            wsai_code_config=wsai_code_config,
        ...
        assert_sandbox_config(
            wsai_code_config.sandbox,
            runtime_container_image='ghcr.io/wsai_code/runtime:mock-nikolaik',
        )
...
def test_setup_sandbox_config_runtime_only():
    ...
    assert_sandbox_config(
        wsai_code_config.sandbox, runtime_container_image=runtime_image
    )
...
def test_setup_sandbox_config_experimental():
    ...
    with mock.patch('wsai_code.__version__', 'mock'):
        ...
        assert_sandbox_config(wsai_code_config.sandbox, runtime_container_image=None)


@mock.patch('wsai_code.resolver.issue_resolver.os.getuid', return_value=0)
@mock.patch('wsai_code.resolver.issue_resolver.get_unique_uid', return_value=1001)
def test_setup_sandbox_config_gitlab_ci(mock_get_unique_uid, mock_getuid):
    ...
    with mock.patch('wsai_code.__version__', 'mock'):
        with mock.patch.object(IssueResolver, 'GITLAB_CI', True):
            ...
            assert_sandbox_config(
                wsai_code_config.sandbox, local_runtime_url='http://localhost'
            )


@mock.patch('wsai_code.resolver.issue_resolver.os.getuid', return_value=1000)
...
@mock.patch('wsai_code.events.observation.CmdOutputObservation')
@mock.patch('wsai_code.runtime.base.Runtime')
