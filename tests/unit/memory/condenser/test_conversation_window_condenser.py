...
from wsaicode.events import EventSource
from wsaicode.events.action import CmdRunAction, MessageAction, RecallAction
from wsaicode.events.action.agent import CondensationAction
...
def create_events(event_data):
    ...
    from wsaicode.events.action import CmdRunAction, RecallAction
    from wsaicode.events.observation import CmdOutputObservation, RecallObservation
...
def test_only_dangling_observations_in_recent_slice(condenser_fixture):
    ...
    # Forgotten IDs: [5, 6]
    with patch(
        'wsaicode.memory.condenser.impl.conversation_window_condenser.logger.warning'
    ) as mock_log_warning:
        condensation = condenser.get_condensation(view)
