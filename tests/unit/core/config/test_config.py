...
def default_config(monkeypatch):
    # Fixture to provide a default WSAICodeConfig instance
    yield WSAICodeConfig()


def test_compat_env_to_config(monkeypatch, setup_env):
    # Use `monkeypatch` to set environment variables for this specific test
    monkeypatch.setenv('SANDBOX_VOLUMES', '/repos/wsaicode/workspace:/workspace:rw')
    ...
    assert config.sandbox.volumes == '/repos/wsaicode/workspace:/workspace:rw'
    # Check that the old parameters are set for backward compatibility
    assert config.workspace_base == os.path.abspath('/repos/wsaicode/workspace')
    assert config.workspace_mount_path == os.path.abspath('/repos/wsaicode/workspace')
...
[llm]
# Not set in `wsaicode/core/schema/config.py`
invalid_field = "test"
model = "gpt-4"
...
""")
    ...
    wsaicode_logger.addHandler(handler)
    ...
    try:
        ...
        wsaicode_logger.removeHandler(handler)
...
def test_agent_config_condenser_with_no_enabled():
    ...
    config = WSAICodeConfig(enable_default_condenser=False)
...
def test_api_keys_repr_str():
    ...
    # Check that no other attrs in WSAICodeConfig have 'key' or 'token' in their name
    # This will fail when new attrs are added, and attract attention
    known_key_token_attrs_app = [
        'search_api_key',
    ]
    for attr_name in WSAICodeConfig.model_fields.keys():
        if (
            not attr_name.startswith('__')
            and attr_name not in known_key_token_attrs_app
        ):
            assert 'key' not in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'key' in WSAICodeConfig"
            )
            assert 'token' not in attr_name.lower() or 'tokens' in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'token' in WSAICodeConfig"
            )
...
"""
    ...
    app_config = load_wsaicode_config(config_file=temp_toml_file)
