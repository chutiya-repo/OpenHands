...
def default_config(monkeypatch):
    # Fixture to provide a default WSAI CODEConfig instance
    yield WSAI CODEConfig()


def test_compat_env_to_config(monkeypatch, setup_env):
    # Use `monkeypatch` to set environment variables for this specific test
    monkeypatch.setenv('SANDBOX_VOLUMES', '/repos/wsai_code/workspace:/workspace:rw')
    ...
    assert config.sandbox.volumes == '/repos/wsai_code/workspace:/workspace:rw'
    # Check that the old parameters are set for backward compatibility
    assert config.workspace_base == os.path.abspath('/repos/wsai_code/workspace')
    assert config.workspace_mount_path == os.path.abspath('/repos/wsai_code/workspace')
...
def test_defaults_dict_after_updates(default_config):
    ...
    updated_config = WSAI CODEConfig()
...
[llm]
# Not set in `wsai_code/core/schema/config.py`
invalid_field = "test"
model = "gpt-4"
...
""")
    ...
    wsai_code_logger.addHandler(handler)
    ...
    try:
        ...
        wsai_code_logger.removeHandler(handler)
...
def test_agent_config_condenser_with_no_enabled():
    ...
    config = WSAI CODEConfig(enable_default_condenser=False)
...
def test_api_keys_repr_str():
    ...
    # Check that no other attrs in WSAI CODEConfig have 'key' or 'token' in their name
    # This will fail when new attrs are added, and attract attention
    known_key_token_attrs_app = [
        'search_api_key',
    ]
    for attr_name in WSAI CODEConfig.model_fields.keys():
        if (
            not attr_name.startswith('__')
            and attr_name not in known_key_token_attrs_app
        ):
            assert 'key' not in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'key' in WSAI CODEConfig"
            )
            assert 'token' not in attr_name.lower() or 'tokens' in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'token' in WSAI CODEConfig"
            )
...
"""
    ...
    app_config = load_wsai_code_config(config_file=temp_toml_file)
