...
from wsai_code.core.logger import (
    LOG_JSON_LEVEL_KEY,
    WSAI CODELoggerAdapter,
    json_log_handler,
)
from wsai_code.core.logger import wsai_code_logger as wsai_code_logger
...
def json_handler():
    ...
    wsai_code_logger.addHandler(json_handler)
    yield wsai_code_logger, stream
    wsai_code_logger.removeHandler(json_handler)
...
def test_app_config_attributes_masking(test_handler):
    logger, stream = test_handler
    app_config = WSAI CODEConfig(search_api_key='search-xyz789')
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict('wsai_code.core.logger.os.environ', environ, clear=True):
...
class TestJsonOutput:
    ...
    def test_extra_fields_from_adapter(self, json_handler):
        logger, string_io = json_handler
        subject = WSAI CODELoggerAdapter(logger, extra={'context_field': '..val..'})
    ...
    def test_extra_fields_from_adapter_can_override(self, json_handler):
        logger, string_io = json_handler
        subject = WSAI CODELoggerAdapter(logger, extra={'override': 'a'})
