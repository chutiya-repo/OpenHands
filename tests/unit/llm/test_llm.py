...
from wsaicode.core.exceptions import LLMNoResponseError, OperationCancelled
from wsaicode.core.message import Message, TextContent
from wsaicode.llm.async_llm import AsyncLLM
from wsaicode.llm.llm import LLM
from wsaicode.llm.metrics import Metrics, TokenUsage
from wsaicode.llm.streaming_llm import StreamingLLM
...
def mock_logger(monkeypatch):
    ...
    monkeypatch.setattr('wsaicode.llm.debug_mixin.llm_prompt_logger', mock_logger)
    monkeypatch.setattr('wsaicode.llm.debug_mixin.llm_response_logger', mock_logger)
    monkeypatch.setattr('wsaicode.llm.llm.logger', mock_logger)
    return mock_logger
...
@patch('wsaicode.llm.llm.litellm.get_model_info')
...
@patch('wsaicode.llm.llm.litellm_completion')
...
@patch('wsaicode.llm.llm.litellm.get_model_info')
@patch('wsaicode.llm.llm.httpx.get')
...
@patch('wsaicode.llm.llm.litellm.token_counter')
...
@patch('wsaicode.llm.llm.litellm.token_counter')
@patch('wsaicode.llm.llm.create_pretrained_tokenizer')
...
@patch('wsaicode.llm.llm.httpx.get')
@patch('wsaicode.llm.llm.litellm.get_model_info')
def test_wsaicode_provider_rewrite_and_caching_prompt(
    ...
    config = LLMConfig(
        model='wsaicode/claude-3.7-sonnet', api_key='k', caching_prompt=True
    )
    llm = LLM(config, service_id='svc')
...
@patch('wsaicode.llm.async_llm.litellm_acompletion')
@pytest.mark.asyncio
...
@patch('wsaicode.llm.streaming_llm.AsyncLLM._call_acompletion')
@pytest.mark.asyncio
