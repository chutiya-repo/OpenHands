...
from wsai_code.core.exceptions import LLMNoResponseError, OperationCancelled
from wsai_code.core.message import Message, TextContent
...
def mock_logger(monkeypatch):
    ...
    monkeypatch.setattr('wsai_code.llm.debug_mixin.llm_prompt_logger', mock_logger)
    monkeypatch.setattr('wsai_code.llm.debug_mixin.llm_response_logger', mock_logger)
    monkeypatch.setattr('wsai_code.llm.llm.logger', mock_logger)
    return mock_logger
...
@patch('wsai_code.llm.llm.litellm.get_model_info')
...
@patch('wsai_code.llm.llm.litellm_completion')
...
@patch('wsai_code.llm.llm.litellm.get_model_info')
@patch('wsai_code.llm.llm.httpx.get')
...
@patch('wsai_code.llm.llm.litellm.token_counter')
...
@patch('wsai_code.llm.llm.litellm.token_counter')
@patch('wsai_code.llm.llm.create_pretrained_tokenizer')
...
@patch('wsai_code.llm.llm.httpx.get')
@patch('wsai_code.llm.llm.litellm.get_model_info')
def test_wsai_code_provider_rewrite_and_caching_prompt(
    ...
    config = LLMConfig(
        model='wsai_code/claude-3.7-sonnet', api_key='k', caching_prompt=True
    )
    llm = LLM(config, service_id='svc')
...
@patch('wsai_code.llm.async_llm.litellm_acompletion')
@pytest.mark.asyncio
...
@patch('wsai_code.llm.streaming_llm.AsyncLLM._call_acompletion')
@pytest.mark.asyncio
