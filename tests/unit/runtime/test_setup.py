...
from wsaicode.events.action import CmdRunAction, FileReadAction
from wsaicode.events.event import EventSource
from wsaicode.events.observation import ErrorObservation, FileReadObservation
...
def test_maybe_run_setup_script_executes_action():
    ...
    runtime.read.return_value = FileReadObservation(
        content="#!/bin/bash\necho 'test'", path='.wsaicode/setup.sh'
    )
    ...
    runtime.read.assert_called_once_with(FileReadAction(path='.wsaicode/setup.sh'))
    ...
    assert isinstance(action, CmdRunAction)
    assert (
        action.command == 'chmod +x .wsaicode/setup.sh && source .wsaicode/setup.sh'
    )
