...
import wsaicode
from wsaicode import __version__ as oh_version
from wsaicode.core.logger import wsaicode_logger as logger
from wsaicode.runtime.builder.docker import DockerRuntimeBuilder
from wsaicode.runtime.utils.runtime_build import (
    BuildFromImageType,
...
def _check_source_code_in_dir(temp_dir):
    ...
    # The source code should only include the `wsaicode` folder,
    ...
    assert os.path.exists(os.path.join(code_dir, 'wsaicode'))
    assert os.path.isdir(os.path.join(code_dir, 'wsaicode'))
    ...
    assert _pyproject_version == version('wsaicode-ai')
...
def test_prep_build_folder(temp_dir):
    ...
    # make sure that the code (wsaicode/) and microagents folder were copied
...
def test_generate_dockerfile_build_from_scratch():
    ...
    assert (
        'COPY --chown=wsaicode:wsaicode ./code/wsaicode /wsaicode/code/wsaicode'
        in dockerfile_content
    )
    assert (
        '/wsaicode/micromamba/bin/micromamba run -n wsaicode poetry install'
        in dockerfile_content
    )
...
def test_generate_dockerfile_channel_alias(monkeypatch):
    ...
    install_snippet = (
        '/wsaicode/micromamba/bin/micromamba install -n wsaicode -c conda-forge'
    )
...
def live_docker_image():
    ...
    RUN mkdir -p /wsaicode

    FROM intermediate AS final
    RUN echo "Hello, WSAI CODE!" > /wsaicode/hello.txt
    """
