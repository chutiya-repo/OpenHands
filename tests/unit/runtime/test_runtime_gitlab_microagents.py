...
class MockRuntime(Runtime):
    ...
    def read(self, action):
        """Mock read method."""
        from wsaicode.events.observation import ErrorObservation
    ...
    def _load_microagents_from_directory(self, directory: Path, source: str):
        ...
        for md_file in directory.rglob('*.md'):
            ...
            from wsaicode.microagent.types import MicroagentMetadata, MicroagentType
    ...
    def run_ipython(self, action):
        from wsaicode.events.observation import IPythonRunCellObservation

        return IPythonRunCellObservation(content='', code='')

    def edit(self, action):
        from wsaicode.events.observation import FileEditObservation

        return FileEditObservation(content='', path='')

    def browse(self, action):
        from wsaicode.events.observation import BrowserObservation
    ...
    def write(self, action):
        from wsaicode.events.observation import FileWriteObservation
    ...
    def call_tool_mcp(self, action):
        from wsaicode.events.observation import MCPObservation

        return MCPObservation(content='', tool='', result='')


def create_test_microagents(base_dir: Path, config_dir_name: str = '.wsaicode'):
...
def test_is_gitlab_repository_github(temp_workspace):
    ...
    with patch('wsaicode.runtime.base.ProviderHandler') as mock_handler_class:
        mock_handler = MagicMock()
        mock_handler_class.return_value = mock_handler

        with patch('wsaicode.runtime.base.call_async_from_sync') as mock_async:
...
def test_get_microagents_from_org_or_user_github(temp_workspace):
    """Test that GitHub repositories only try .wsaicode directory."""
    runtime = MockRuntime(temp_workspace)
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=False):
        with patch.object(runtime, '_is_azure_devops_repository', return_value=False):
            ...
            with patch('wsaicode.runtime.base.call_async_from_sync') as mock_async:
                ...
                # Should only try .wsaicode, not wsaicode-config
                assert len(result) == 0
                # Check that only one attempt was made (for .wsaicode)
                assert mock_async.call_count == 1


def test_get_microagents_from_org_or_user_gitlab_success_with_config(temp_workspace):
    """Test that GitLab repositories use wsaicode-config and succeed."""
    runtime = MockRuntime(temp_workspace)

    # Create a mock org directory with microagents
    org_dir = temp_workspace / 'org_wsaicode_owner'
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=True):
        with patch.object(runtime, '_is_azure_devops_repository', return_value=False):
            # Mock successful cloning for wsaicode-config
            with patch('wsaicode.runtime.base.call_async_from_sync') as mock_async:
                mock_async.return_value = (
                    'https://gitlab.com/owner/wsaicode-config.git'
                )
                ...
                # Should succeed with wsaicode-config
                assert len(result) >= 0  # May be empty if no microagents found
                # Should only try once for wsaicode-config
                assert mock_async.call_count == 1


def test_get_microagents_from_org_or_user_gitlab_failure(temp_workspace):
    """Test that GitLab repositories handle failure gracefully when wsaicode-config doesn't exist."""
    runtime = MockRuntime(temp_workspace)

    # Mock the provider detection to return GitLab
    with patch.object(runtime, '_is_gitlab_repository', return_value=True):
        with patch.object(runtime, '_is_azure_devops_repository', return_value=False):
            # Mock the _get_authenticated_git_url to fail for wsaicode-config
            with patch('wsaicode.runtime.base.call_async_from_sync') as mock_async:
                mock_async.side_effect = Exception('wsaicode-config not found')
...
def test_get_microagents_from_selected_repo_gitlab_uses_wsaicode(temp_workspace):
    """Test that GitLab repositories use .wsaicode directory for repository-specific microagents."""
    runtime = MockRuntime(temp_workspace)
    ...
    # Create microagents in .wsaicode directory
    create_test_microagents(repo_dir, '.wsaicode')
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=True):
        ...
        with patch.object(runtime, 'get_microagents_from_org_or_user', return_value=[]):
            ...
            # Should find microagents from .wsaicode directory
            # The exact assertion depends on the mock implementation
            # At minimum, it should not raise an exception
            assert isinstance(result, list)


def test_get_microagents_from_selected_repo_github_only_wsaicode(temp_workspace):
    """Test that GitHub repositories only check .wsaicode directory."""
    runtime = MockRuntime(temp_workspace)
    ...
    create_test_microagents(repo_dir, 'wsaicode-config')
    create_test_microagents(repo_dir, '.wsaicode')
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=False):
        ...
        with patch.object(runtime, 'get_microagents_from_org_or_user', return_value=[]):
            ...
            # Should only check .wsaicode directory, not wsaicode-config
            assert isinstance(result, list)
