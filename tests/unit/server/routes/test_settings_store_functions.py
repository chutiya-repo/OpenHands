...
from wsai_code.server.routes.secrets import (
    app as secrets_router,
)
from wsai_code.server.routes.secrets import (
    check_provider_tokens,
)
from wsai_code.server.routes.settings import store_llm_settings
...
from wsai_code.storage.secrets.file_secrets_store import FileSecretsStore
...
def test_client():
    ...
    with (
        ...
        patch('wsai_code.server.dependencies._SESSION_API_KEY', None),
        patch(
            'wsai_code.server.routes.secrets.check_provider_tokens',
...
def file_secrets_store(temp_dir):
    ...
    with patch(
        'wsai_code.storage.secrets.file_secrets_store.FileSecretsStore.get_instance',
...
async def test_check_provider_tokens_valid():
    ...
    with patch(
        'wsai_code.server.routes.secrets.validate_provider_token'
    ) as mock_validate:
