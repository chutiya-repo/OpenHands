...
from wsai_code.app_server.app_conversation.app_conversation_router import (
...
from wsai_code.sdk.conversation.state import ConversationExecutionStatus
from wsai_code.sdk.workspace.models import FileOperationResult
from wsai_code.sdk.workspace.remote.async_remote_workspace import (
    AsyncRemoteWorkspace,
)
from wsai_code.server.data_models.conversation_info import ConversationInfo
from wsai_code.server.data_models.conversation_info_result_set import (
...
from wsai_code.server.routes.manage_conversations import app as conversation_app
from wsai_code.server.types import LLMAuthenticationError, MissingSettingsError
from wsai_code.server.user_auth.user_auth import AuthType
from wsai_code.storage.data_models.conversation_metadata import (
    ConversationMetadata,
    ConversationTrigger,
)
from wsai_code.storage.data_models.conversation_status import ConversationStatus
from wsai_code.storage.locations import get_conversation_metadata_filename
...
def _patch_store():
    ...
    with patch(
        'wsai_code.storage.conversation.file_conversation_store.get_file_store',
        MagicMock(return_value=file_store),
    ):
        with patch(
            'wsai_code.server.routes.manage_conversations.conversation_manager.file_store',
            file_store,
        ):
            yield
...
def provider_handler_mock():
    with patch(
        'wsai_code.server.routes.manage_conversations.ProviderHandler'
    ) as mock_cls:
...
async def test_search_conversations():
    with _patch_store():
        with patch(
            'wsai_code.server.routes.manage_conversations.config'
        ) as mock_config:
            mock_config.conversation_max_age_seconds = 864000  # 10 days
            with patch(
                'wsai_code.server.routes.manage_conversations.conversation_manager'
            ) as mock_manager:
                ...
                with patch(
                    'wsai_code.server.routes.manage_conversations.datetime'
                ) as mock_datetime:
...
async def test_new_conversation_success(provider_handler_mock):
    ...
    with _patch_store():
        ...
        with patch(
            'wsai_code.server.routes.manage_conversations.create_new_conversation'
...
async def test_new_conversation_with_suggested_task(provider_handler_mock):
    ...
    with _patch_store():
        ...
        with patch(
            ...
            with patch(
                'wsai_code.integrations.service_types.SuggestedTask.get_prompt_for_task'
            ) as mock_get_prompt:
...
async def test_delete_conversation():
    with _patch_store():
        ...
        with patch(
            'wsai_code.server.routes.manage_conversations.ConversationStoreImpl.get_instance'
            ...
            with patch(
                ...
                with patch(
                    'wsai_code.server.routes.manage_conversations.get_runtime_cls'
...
async def test_delete_v1_conversation_success():
    ...
    with patch(
        'wsai_code.server.routes.manage_conversations.app_conversation_service_dependency'
    ) as mock_service_dep:
        ...
        with patch(
            'wsai_code.server.routes.manage_conversations.app_conversation_info_service_dependency'
            ...
            with patch(
                'wsai_code.server.routes.manage_conversations.sandbox_service_dependency'
...
async def test_read_conversation_file_success():
    ...
    with patch(
        'wsai_code.app_server.app_conversation.app_conversation_router.AsyncRemoteWorkspace'
        ...
        with patch(
            'wsai_code.app_server.app_conversation.app_conversation_router.tempfile.NamedTemporaryFile'
        ) as mock_tempfile:
            ...
            with patch('builtins.open', create=True) as mock_open:
                ...
                with patch(
                    'wsai_code.app_server.app_conversation.app_conversation_router.os.unlink'
                ) as mock_unlink:
                    # Call the endpoint
