...
from wsaicode.events import Event, EventSource, EventStream, EventStreamSubscriber
from wsaicode.events.action import ChangeAgentStateAction, CmdRunAction, MessageAction
from wsaicode.events.action.agent import CondensationAction, RecallAction
...
from wsaicode.events.serialization import event_to_dict
from wsaicode.llm import LLM
from wsaicode.llm.llm_registry import LLMRegistry, RegistryEvent
from wsaicode.llm.metrics import Metrics, TokenUsage
from wsaicode.memory.condenser.condenser import Condensation
from wsaicode.memory.condenser.impl.conversation_window_condenser import (
    ConversationWindowCondenser,
)
from wsaicode.memory.memory import Memory
from wsaicode.memory.view import View
...
async def test_run_controller_with_fatal_error(
    ...
    with patch('wsaicode.core.main.create_agent', return_value=mock_agent):
...
async def test_context_window_exceeded_error_handling(
    ...
    config = WSAICodeConfig(max_iterations=max_iterations)
...
async def test_run_controller_with_context_window_exceeded_with_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = WSAICodeConfig(max_iterations=5)
    mock_runtime.config = copy.deepcopy(config)
...
async def test_run_controller_with_context_window_exceeded_without_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = WSAICodeConfig(max_iterations=3)
    mock_runtime.config = copy.deepcopy(config)
    try:
        # Mock the create_agent function to return our mock agent
        with patch('wsaicode.core.main.create_agent', return_value=mock_agent):
