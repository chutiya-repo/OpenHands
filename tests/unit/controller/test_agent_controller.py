...
from wsai_code.controller.state.control_flags import (
    BudgetControlFlag,
)
...
from wsai_code.events import Event, EventSource, EventStream, EventStreamSubscriber
from wsai_code.events.action import ChangeAgentStateAction, CmdRunAction, MessageAction
from wsai_code.events.action.agent import CondensationAction, RecallAction
...
from wsai_code.events.observation.empty import NullObservation
from wsai_code.events.serialization import event_to_dict
from wsai_code.llm import LLM
from wsai_code.llm.llm_registry import LLMRegistry, RegistryEvent
from wsai_code.llm.metrics import Metrics, TokenUsage
from wsai_code.memory.condenser.condenser import Condensation
...
async def test_run_controller_with_fatal_error(
    ...
    with patch('wsai_code.core.main.create_agent', return_value=mock_agent):
...
async def test_context_window_exceeded_error_handling(
    ...
    config = WSAI CODEConfig(max_iterations=max_iterations)
...
async def test_run_controller_with_context_window_exceeded_with_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = WSAI CODEConfig(max_iterations=5)
    mock_runtime.config = copy.deepcopy(config)
...
async def test_run_controller_with_context_window_exceeded_without_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = WSAI CODEConfig(max_iterations=3)
    mock_runtime.config = copy.deepcopy(config)
    try:
        # Mock the create_agent function to return our mock agent
        with patch('wsai_code.core.main.create_agent', return_value=mock_agent):
