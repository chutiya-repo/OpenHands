...
from wsaicode.integrations.service_types import OwnerType, Repository
from wsaicode.integrations.service_types import ProviderType as ServiceProviderType
from wsaicode.integrations.utils import validate_provider_token
from wsaicode.resolver.interfaces.bitbucket import BitbucketIssueHandler
from wsaicode.resolver.interfaces.issue import Issue
from wsaicode.resolver.interfaces.issue_definitions import ServiceContextIssue
from wsaicode.resolver.send_pull_request import PR_SIGNATURE, send_pull_request
from wsaicode.runtime.base import Runtime
from wsaicode.server.routes.secrets import check_provider_tokens
from wsaicode.server.settings import POSTProviderModel
...
@patch('wsaicode.resolver.send_pull_request.ServiceContextIssue')
@patch('wsaicode.resolver.send_pull_request.BitbucketIssueHandler')
...
def test_send_pull_request_bitbucket(
    ...
    mock_service.get_branch_name.return_value = 'wsaicode-fix-123'
    ...
    mock_service.create_pull_request.assert_called_once_with(
        {
            ...
            'source_branch': 'wsaicode-fix-123',
            'target_branch': 'main',
            'draft': False,
        }
    )
...
class TestBitbucketProviderDomain(unittest.TestCase):
    ...
    @patch('wsaicode.runtime.base.Runtime.__abstractmethods__', set())
    @patch(
        'wsaicode.runtime.utils.edit.FileEditRuntimeMixin.__init__', return_value=None
    )
    @patch('wsaicode.runtime.base.ProviderHandler')
    @pytest.mark.asyncio
...
async def test_validate_provider_token_with_bitbucket_token():
    ...
    with (
        patch('wsaicode.integrations.utils.GitHubService') as mock_github_service,
        patch('wsaicode.integrations.utils.GitLabService') as mock_gitlab_service,
        patch(
            'wsaicode.integrations.utils.BitBucketService'
...
async def test_check_provider_tokens_with_only_bitbucket():
    ...
    with patch(
        'wsaicode.server.routes.secrets.validate_provider_token', mock_validate
    ):
...
@patch('wsaicode.core.setup.call_async_from_sync')
@patch('wsaicode.core.setup.get_file_store')
@patch('wsaicode.core.setup.EventStream')
def test_initialize_repository_for_runtime_with_bitbucket_token(
    ...
    with patch.dict(os.environ, {'BITBUCKET_TOKEN': 'username:app_password'}):
        result = initialize_repository_for_runtime(
            runtime=mock_runtime, selected_repository='wsaicode/test-repo'
        )
    ...
    assert args[3] == 'wsaicode/test-repo'  # selected_repository
