...
from wsaicode.agent_server.models import (
    SendMessageRequest,
...
from wsaicode.app_server.app_conversation.live_status_app_conversation_service import (
    LiveStatusAppConversationService,
)
from wsaicode.app_server.sandbox.sandbox_models import (
    AGENT_SERVER,
    ExposedUrl,
    SandboxInfo,
    SandboxStatus,
)
from wsaicode.app_server.sandbox.sandbox_spec_models import SandboxSpecInfo
...
from wsaicode.sdk import Agent, Event
from wsaicode.sdk.llm import LLM
from wsaicode.sdk.secret import LookupSecret, StaticSecret
from wsaicode.sdk.workspace import LocalWorkspace
...
class TestLiveStatusAppConversationService:
    ...
    def setup_method(self):
        ...
        self.service = LiveStatusAppConversationService(
            ...
            wsaicode_provider_base_url='https://provider.example.com',
    ...
    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_wsaicode_model_prefers_user_base_url(self):
        """wsaicode/* model uses user.llm_base_url when provided."""
        # Arrange
        self.mock_user.llm_model = 'wsaicode/special'
    ...
    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_wsaicode_model_uses_provider_default(self):
        """wsaicode/* model falls back to configured provider base URL."""
        # Arrange
        self.mock_user.llm_model = 'wsaicode/default'
    ...
    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_wsaicode_model_no_base_urls(self):
        """wsaicode/* model sets base_url to None when no sources available."""
        # Arrange
        self.mock_user.llm_model = 'wsaicode/default'
        self.mock_user.llm_base_url = None
        self.service.wsaicode_provider_base_url = None
        self.mock_user_context.get_mcp_api_key.return_value = None
        ...
        # Assert
        assert llm.base_url == 'https://llm-proxy.app.wsai-code.dev/'

    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_non_wsaicode_model_ignores_provider(self):
        """Non-wsaicode model ignores provider base URL and uses user base URL."""
        # Arrange
        self.mock_user.llm_model = 'gpt-4'
        self.mock_user.llm_base_url = 'https://user-llm.example.com'
        self.service.wsaicode_provider_base_url = 'https://provider.example.com'
        self.mock_user_context.get_mcp_api_key.return_value = None
    ...
    @patch(
        'wsaicode.app_server.app_conversation.live_status_app_conversation_service.get_planning_tools'
    )
    @patch(
        'wsaicode.app_server.app_conversation.app_conversation_service_base.AppConversationServiceBase._create_condenser'
    )
    @patch(
        'wsaicode.app_server.app_conversation.live_status_app_conversation_service.format_plan_structure'
    )
    def test_create_agent_with_context_planning_agent(
        ...
        # Act
        with patch(
            'wsaicode.app_server.app_conversation.live_status_app_conversation_service.Agent'
        ) as mock_agent_class:
    ...
    @patch(
        'wsaicode.app_server.app_conversation.live_status_app_conversation_service.get_default_tools'
    )
    @patch(
        'wsaicode.app_server.app_conversation.app_conversation_service_base.AppConversationServiceBase._create_condenser'
    )
    ...
    @pytest.mark.asyncio
    @patch(
        'wsaicode.app_server.app_conversation.live_status_app_conversation_service.ExperimentManagerImpl'
    )
    async def test_finalize_conversation_request_with_skills(
        ...
        mock_llm = Mock(spec=LLM)
        mock_llm.model = 'gpt-4'  # Non-wsaicode model, so no metadata update
    ...
    async def test_finalize_conversation_request_skills_loading_fails(
        ...
        # Act
        with patch(
            'wsaicode.app_server.app_conversation.live_status_app_conversation_service._logger'
        ) as mock_logger:
            result = await self.service._finalize_conversation_request(
                mock_agent,
                None,
    ...
    @patch(
        'wsaicode.app_server.app_conversation.live_status_app_conversation_service.AsyncRemoteWorkspace'
    )
    @patch(
        'wsaicode.app_server.app_conversation.live_status_app_conversation_service.ConversationInfo'
    )
    ...
    async def test_configure_llm_and_mcp_with_custom_sse_servers(self):
        ...
        from wsaicode.core.config.mcp_config import MCPConfig, MCPSSEServerConfig
    ...
    async def test_configure_llm_and_mcp_with_custom_shttp_servers(self):
        ...
        # Arrange
        from wsaicode.core.config.mcp_config import MCPConfig, MCPSHTTPServerConfig
