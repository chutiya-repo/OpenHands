...
from wsaicode.sdk.conversation.conversation_stats import ConversationStats
from wsaicode.sdk.event import ConversationStateUpdateEvent
from wsaicode.sdk.llm.utils.metrics import Metrics, TokenUsage
...
class TestProcessStatsEvent:
    ...
    async def test_process_stats_event_service_error_handled(
        ...
        with (
            ...
            patch(
                'wsaicode.app_server.app_conversation.sql_app_conversation_info_service.logger'
            ) as mock_logger,
        ):
            await service.process_stats_event(
...
class TestOnEventStatsProcessing:
    ...
    async def test_on_event_processes_stats_events(self):
        ...
        from wsaicode.app_server.event_callback.webhook_router import on_event
        ...
        with (
            patch(
                'wsaicode.app_server.event_callback.webhook_router.valid_sandbox',
                return_value=mock_sandbox,
            ),
            patch(
                'wsaicode.app_server.event_callback.webhook_router.valid_conversation',
                return_value=mock_app_conversation_info,
            ),
            patch(
                'wsaicode.app_server.event_callback.webhook_router._run_callbacks_in_bg_and_close'
            ) as mock_callbacks,
        ):
            await on_event(
                events=events,
