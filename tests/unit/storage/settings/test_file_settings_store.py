...
async def test_load_nonexistent_data(file_settings_store):
    with patch(
        'wsai_code.storage.data_models.settings.load_wsai_code_config',
        MagicMock(return_value=WSAI CODEConfig()),
    ):
...
async def test_get_instance():
    config = WSAI CODEConfig(file_store='local', file_store_path='/test/path')

    with patch(
        'wsai_code.storage.settings.file_settings_store.get_file_store'
    ) as mock_get_store:
