...
async def test_load_nonexistent_data(file_settings_store):
    with patch(
        ...
        MagicMock(return_value=WSAICodeConfig()),
    ):
...
async def test_get_instance():
    config = WSAICodeConfig(file_store='local', file_store_path='/test/path')

    with patch(
        'wsaicode.storage.settings.file_settings_store.get_file_store'
    ) as mock_get_store:
