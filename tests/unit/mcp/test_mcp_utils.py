...
import wsai_code.mcp.utils
from wsai_code.core.config.mcp_config import MCPSSEServerConfig, MCPStdioServerConfig
...
async def test_create_mcp_clients_empty():
    ...
    clients = await wsai_code.mcp.utils.create_mcp_clients([], [])
    assert clients == []


@pytest.mark.asyncio
@patch('wsai_code.mcp.utils.MCPClient')
async def test_create_mcp_clients_success(mock_mcp_client):
    ...
    clients = await wsai_code.mcp.utils.create_mcp_clients(server_configs, [])
...
def test_convert_mcp_clients_to_tools_empty():
    ...
    tools = wsai_code.mcp.utils.convert_mcp_clients_to_tools(None)
    assert tools == []

    tools = wsai_code.mcp.utils.convert_mcp_clients_to_tools([])
    assert tools == []
...
async def test_call_tool_mcp_no_clients():
    ...
    with pytest.raises(ValueError, match='No MCP clients found'):
        await wsai_code.mcp.utils.call_tool_mcp([], action)
...
async def test_call_tool_mcp_no_matching_client():
    ...
    with pytest.raises(ValueError, match='No matching MCP agent found for tool name'):
        await wsai_code.mcp.utils.call_tool_mcp([mock_client], action)
...
async def test_call_tool_mcp_success():
    ...
    # Call the function
    observation = await wsai_code.mcp.utils.call_tool_mcp([mock_client], action)
...
@pytest.mark.asyncio
@patch('wsai_code.mcp.utils.create_mcp_clients')
async def test_fetch_mcp_tools_from_config_with_stdio(mock_create_clients):
    ...
    tools = await wsai_code.mcp.utils.fetch_mcp_tools_from_config(
