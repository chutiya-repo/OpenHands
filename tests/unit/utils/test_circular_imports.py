...
class TestCircularImports(unittest.TestCase):
    ...
    def test_no_circular_imports_in_key_modules(self):
        ...
        - wsai_code.utils.prompt
        - wsai_code.agenthub.codeact_agent.tools.bash
        - wsai_code.agenthub.codeact_agent.tools.prompt
        - wsai_code.memory.memory
        - wsai_code.memory.conversation_memory
        """
        ...
        module_paths = {
            'wsai_code.utils.prompt': os.path.join(
                project_root, 'wsai_code/utils/prompt.py'
            ),
            'wsai_code.agenthub.codeact_agent.tools.bash': os.path.join(
                project_root, 'wsai_code/agenthub/codeact_agent/tools/bash.py'
            ),
            'wsai_code.agenthub.codeact_agent.tools.prompt': os.path.join(
                project_root, 'wsai_code/agenthub/codeact_agent/tools/prompt.py'
            ),
            'wsai_code.memory.memory': os.path.join(
                project_root, 'wsai_code/memory/memory.py'
            ),
            'wsai_code.memory.conversation_memory': os.path.join(
                project_root, 'wsai_code/memory/conversation_memory.py'
            ),
        }
                ...
                for line in import_lines:
                    if line.startswith('import '):
                        ...
                        for part in parts:
                            ...
                            if module_part.startswith('wsai_code.'):
    ...
    def test_specific_circular_import_pattern(self):
        ...
        wsai_code.utils.prompt imports from wsai_code.agenthub.codeact_agent.tools.bash
        wsai_code.agenthub.codeact_agent.tools.bash imports from wsai_code.agenthub.codeact_agent.tools.prompt
        wsai_code.agenthub.codeact_agent.tools.prompt imports from wsai_code.utils.prompt
        """
        # Get the project root directory
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../..'))

        # Check if the problematic pattern exists
        prompt_path = os.path.join(project_root, 'wsai_code/utils/prompt.py')
        ...
        prompt_imports_bash = (
            re.search(
                r'from wsai_code\.agenthub\.codeact_agent\.tools\.bash import',
                prompt_code,
            )
            is not None
        )
        bash_imports_tools_prompt = (
            re.search(
                r'from wsai_code\.agenthub\.codeact_agent\.tools\.prompt import',
                bash_code,
            )
            is not None
        )
        tools_prompt_imports_prompt = (
            re.search(r'from wsai_code\.utils\.prompt import', tools_prompt_code)
            is not None
        )
    ...
    def test_detect_circular_imports_in_server_modules(self):
        ...
        - wsai_code.server.shared
        - wsai_code.server.conversation_manager.conversation_manager
        - wsai_code.server.session.agent_session
        - wsai_code.server.session
        - wsai_code.server.session.session
        """
        # Get the project root directory
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../..'))
        ...
        # Map module names to file paths
        module_paths = {
            'wsai_code.server.shared': os.path.join(
                project_root, 'wsai_code/server/shared.py'
            ),
            'wsai_code.server.conversation_manager.conversation_manager': os.path.join(
                project_root,
                'wsai_code/server/conversation_manager/conversation_manager.py',
            ),
            'wsai_code.server.session.agent_session': os.path.join(
                project_root, 'wsai_code/server/session/agent_session.py'
            ),
            'wsai_code.server.session.__init__': os.path.join(
                project_root, 'wsai_code/server/session/__init__.py'
            ),
            'wsai_code.server.session.session': os.path.join(
                project_root, 'wsai_code/server/session/session.py'
            ),
        }
    ...
    def test_detect_circular_imports_in_mcp_modules(self):
        ...
        # Map module names to file paths
        module_paths = {
            'wsai_code.mcp.__init__': os.path.join(
                project_root, 'wsai_code/mcp/__init__.py'
            ),
            'wsai_code.mcp.utils': os.path.join(project_root, 'wsai_code/mcp/utils.py'),
            'wsai_code.memory.memory': os.path.join(
                project_root, 'wsai_code/memory/memory.py'
            ),
        }
    ...
    def test_detect_complex_circular_import_chains(self):
        ...
        modules = [
            ...
            'wsai_code.server.conversation_manager.conversation_manager',
            'wsai_code.server.session.agent_session',
            'wsai_code.server.session.__init__',
            'wsai_code.server.session.session',
            'wsai_code.mcp.__init__',
            'wsai_code.mcp.utils',
        ]
