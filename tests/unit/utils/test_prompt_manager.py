...
from wsaicode.controller.state.control_flags import IterationControlFlag
from wsaicode.controller.state.state import State
from wsaicode.core.message import Message, TextContent
from wsaicode.events.observation.agent import MicroagentKnowledge
from wsaicode.microagent import BaseMicroagent
from wsaicode.utils.prompt import (
...
def prompt_dir(tmp_path):
    # Copy contents from "wsaicode/agenthub/codeact_agent" to the temp directory
    shutil.copytree(
        'wsaicode/agenthub/codeact_agent/prompts', tmp_path, dirs_exist_ok=True
    )
...
def test_jinja2_template_inheritance(prompt_dir):
    ...
    with open(os.path.join(prompt_dir, 'system_prompt.j2'), 'w') as f:
        f.write("""You are WSAI CODE agent, a helpful AI assistant that can interact with a computer to solve tasks.
...
""")
    ...
    assert 'You are WSAI CODE agent' in base_msg
    ...
    assert 'You are WSAI CODE agent' in interactive_msg
    ...
    assert 'You are WSAI CODE agent' in long_horizon_msg
...
def test_prompt_manager_cli_mode_context(prompt_dir):
    ...
    with open(os.path.join(prompt_dir, 'system_prompt.j2'), 'w') as f:
        f.write("""You are WSAI CODE agent.
...
Always be helpful and follow user instructions.
    ...
    assert 'You are WSAI CODE agent' in cli_message
    ...
    assert 'You are WSAI CODE agent' in sandbox_message
    ...
    assert 'You are WSAI CODE agent' in default_message
