# WSAI CODE End-to-End Tests

This directory contains end-to-end tests for the WSAI CODE application. These tests use Playwright to interact with the WSAI CODE UI and verify that the application works correctly.
...
- `--base-url`: Specify the base URL of the WSAI CODE instance under test (default: `http://localhost:12000`)
...
By default, the tests run against `http://localhost:12000`. You can specify a different WSAI CODE instance URL using the `--base-url` option:

```bash
cd tests/e2e
# Run against a remote instance
poetry run pytest test_settings.py::test_github_token_configuration test_conversation.py::test_conversation_start -v --base-url=https://my-wsai_code-instance.com
...
1. Navigates to the WSAI CODE application
...
1. Navigates to the WSAI CODE application (assumes GitHub token is already configured)
2. Selects the "wsai_code-agent/WSAI CODE" repository
...
A simple test (`test_simple_browser_navigation`) that just navigates to the WSAI CODE GitHub repository to verify the browser setup works correctly.
...
2. Check the logs in `/tmp/wsai_code-e2e-test.log` and `/tmp/wsai_code-e2e-build.log`
3. Verify that the WSAI CODE application is running correctly
