...
def test_headless_mode_with_dummy_agent_no_browser():
    """E2E test: build a docker image from python:3.13, install wsai_code from source,
    ...
    ENV RUNTIME=local
    ENV RUN_AS_WSAI_CODE=false
    ENV ENABLE_BROWSER=false
    ENV AGENT_ENABLE_BROWSING=false
    ENV SKIP_DEPENDENCY_CHECK=1
    CMD ["python", "-m", "wsai_code.core.main", "-c", "DummyAgent", "-t", "Hello world"]
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        image_tag = 'wsai_code-e2e-local-runtime-test'
        build_cmd = [
            'docker',
            'build',
            '-t',
