...
def test_simple_cmd_ipython_and_fileop(temp_dir, runtime_cls, run_as_wsai_code):
    ...
    assert obs.content.strip() == (
        ...
        '[Jupyter Python interpreter: /wsai_code/poetry/wsai_code-ai-5O4_aCHf-py3.12/bin/python]'
    )
...
def test_ipython_multi_user(temp_dir, runtime_cls, run_as_wsai_code):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_wsai_code)
    ...
    assert obs.exit_code == 0
    if run_as_wsai_code:
        # -rw-r--r-- 1 wsai_code root 13 Jul 28 03:53 test.txt
        assert 'wsai_code' in obs.content.split('\r\n')[0]
    else:
...
def test_ipython_package_install(temp_dir, runtime_cls, run_as_wsai_code):
...
def test_ipython_file_editor_permissions_as_wsai_code(temp_dir, runtime_cls):
    """Test file editor permission behavior when running as different users."""
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_wsai_code=True)
    ...
    # Try to view the file as wsai_code user - should fail with permission denied
    ...
    # Try to edit the file as wsai_code user - should fail with permission denied
    ...
    # Try to use file editor in wsai_code sandbox directory - should work
    test_code = """
# Create file
