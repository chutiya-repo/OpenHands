...
from wsaicode.core.config import MCPConfig
from wsaicode.core.config.mcp_config import MCPStdioServerConfig
from wsaicode.mcp.utils import add_mcp_tools_to_agent
...
from wsaicode.microagent.types import MicroagentType


def _create_test_microagents(test_dir: str):
    """Create test microagent files in the given directory."""
    microagents_dir = Path(test_dir) / '.wsaicode' / 'microagents'
...
"""
    (Path(test_dir) / '.wsaicode_instructions').write_text(legacy_instructions)
...
def test_load_microagents_with_trailing_slashes(
    ...
    try:
        ...
        assert len(repo_agents) == 2  # repo.md + .wsaicode_instructions
...
def test_load_microagents_with_selected_repo(temp_dir, runtime_cls, run_as_wsaicode):
...
def test_load_microagents_with_missing_files(temp_dir, runtime_cls, run_as_wsaicode):
    """Test loading microagents when some files are missing."""
    # Create only repo.md, no other files
    microagents_dir = Path(temp_dir) / '.wsaicode' / 'microagents'
    microagents_dir.mkdir(parents=True, exist_ok=True)
...
def test_default_tools_microagent_exists():
    ...
    project_root = os.path.dirname(wsaicode.__file__)
...
async def test_add_mcp_tools_from_microagents():
    ...
    with patch(
        'wsaicode.mcp.utils.fetch_mcp_tools_from_config',
        new=AsyncMock(return_value=[mock_tool]),
    ):
        # Call the function with the WSAICodeConfig instead of MCPConfig
