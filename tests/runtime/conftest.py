...
from wsaicode.core.config import MCPConfig, WSAICodeConfig, load_wsaicode_config
...
from wsaicode.runtime.plugins import AgentSkillsRequirement, JupyterRequirement
from wsaicode.runtime.utils.port_lock import find_available_port_with_lock
...
RUN_AS_WSAI_CODE = os.getenv('RUN_AS_WSAI_CODE', 'True').lower() in ['true', '1', 'yes']
test_mount_path = ''
...
def get_run_as_wsaicode() -> list[bool]:
    print(
        '\n\n########################################################################'
    )
    print('USER: ' + 'wsaicode' if RUN_AS_WSAI_CODE else 'root')
    print(
        '########################################################################\n\n'
    )
    return [RUN_AS_WSAI_CODE]
...
@pytest.fixture(scope='module', params=get_run_as_wsaicode())
def run_as_wsaicode(request):
    time.sleep(1)
    return request.param
...
def _load_runtime(
    ...
    config.run_as_wsaicode = run_as_wsaicode
    ...
    llm_registry = LLMRegistry(config=WSAICodeConfig())
